"""Unit tests for ultimate_biological_harmony_bulk.
Template: svc_health_017 | Port: 8238 | Tier: large
...
class TestUltimateBiologicalHarmonyBulkUnit:
    """Unit tests — port 8238 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "ultimate_biological_harmony_bulk"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8238, f"Port mismatch: expected 8238 got {d['port']}"
    ...
    def test_endpoint_get_biological_harmony(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "ultimate_biological_harmony_bulk", \
