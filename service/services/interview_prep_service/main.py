"""
Interview Prep Service — Career Service

Template: svc_catalog_008
Domain: career | Tier: medium | Port: 8220
...
SERVICE_PORT        = config.get('port', 8220)
...
app = FastAPI(
    title="Interview Prep Service API",
    description="Career service | Port 8220 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "interview_prep_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "interview_prep_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "interview_prep_service", "port": SERVICE_PORT,
...
async def jobs():
    ...
    return {"service": "interview_prep_service", "domain": "career",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "interview_prep_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting interview_prep_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
