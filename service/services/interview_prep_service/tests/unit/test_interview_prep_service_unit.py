"""Unit tests for interview_prep_service.
Template: svc_catalog_008 | Port: 8220 | Tier: medium
...
class TestInterviewPrepServiceUnit:
    """Unit tests — port 8220 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "interview_prep_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8220, f"Port mismatch: expected 8220 got {d['port']}"
    ...
    def test_endpoint_get_jobs(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "interview_prep_service", \
