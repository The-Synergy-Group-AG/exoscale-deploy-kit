"""E2E tests for interview_prep_service via gateway.
Port: 8220 | NS: jtp
...
class TestInterviewPrepServiceE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/interview_prep_service/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/interview_prep_service/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_jobs(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/interview_prep_service/jobs", timeout=30.0)
    ...
    def test_gateway_endpoint_applications(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/interview_prep_service/applications", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_interviews(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/interview_prep_service/interviews", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_cv_id(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/interview_prep_service/cv/test-id", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_career_advice(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/interview_prep_service/career/advice", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_cv_generate(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/interview_prep_service/cv/generate", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
