"""Unit tests for test_real_time_communication_validation.
Template: svc_monitoring_016 | Port: 8141 | Tier: small
...
class TestTestRealTimeCommunicationValidationUnit:
    """Unit tests — port 8141 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "test_real_time_communication_validation"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8141, f"Port mismatch: expected 8141 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "test_real_time_communication_validation", \
