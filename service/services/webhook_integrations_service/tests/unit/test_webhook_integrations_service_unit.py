"""Unit tests for webhook_integrations_service.
Template: svc_workflow_012 | Port: 8206 | Tier: medium
...
class TestWebhookIntegrationsServiceUnit:
    """Unit tests — port 8206 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "webhook_integrations_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8206, f"Port mismatch: expected 8206 got {d['port']}"
    ...
    def test_endpoint_get_workflows(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "webhook_integrations_service", \
