"""Unit tests for ai_ethics_enforcer_category.
Template: svc_analytics_009 | Port: 8233 | Tier: large
...
class TestAiEthicsEnforcerCategoryUnit:
    """Unit tests — port 8233 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "ai_ethics_enforcer_category"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8233, f"Port mismatch: expected 8233 got {d['port']}"
    ...
    def test_endpoint_post_ai_process(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "ai_ethics_enforcer_category", \
