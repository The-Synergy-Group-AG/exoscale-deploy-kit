"""Unit tests for docker_compose_test_service.
Template: svc_monitoring_016 | Port: 8130 | Tier: small
...
class TestDockerComposeTestServiceUnit:
    """Unit tests — port 8130 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "docker_compose_test_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8130, f"Port mismatch: expected 8130 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "docker_compose_test_service", \
