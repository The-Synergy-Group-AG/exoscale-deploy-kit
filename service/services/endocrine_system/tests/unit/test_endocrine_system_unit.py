"""Unit tests for endocrine_system.
Template: svc_catalog_008 | Port: 8249 | Tier: medium
...
class TestEndocrineSystemUnit:
    """Unit tests — port 8249 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "endocrine_system"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8249, f"Port mismatch: expected 8249 got {d['port']}"
    ...
    def test_endpoint_get_documents(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "endocrine_system", \
