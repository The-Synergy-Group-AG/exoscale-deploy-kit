"""E2E tests for endocrine_system via gateway.
Port: 8249 | NS: jtp
...
class TestEndocrineSystemE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/endocrine_system/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/endocrine_system/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_documents(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/endocrine_system/documents", timeout=30.0)
    ...
    def test_gateway_endpoint_documents_generate(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/endocrine_system/documents/generate", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_documents_id(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/endocrine_system/documents/test-id", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_documents_id(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/endocrine_system/documents/test-id", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Gateway endpoint /documents/{id} returned {r.status_code}"
            assert r.json() is not None
        except httpx.ConnectError:
    ...
    def test_gateway_endpoint_documents_export(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/endocrine_system/documents/export", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
