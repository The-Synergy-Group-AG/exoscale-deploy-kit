"""
Notifications-Center-Frontend — Frontend Service

Template: svc_frontend_003
Domain: frontend | Tier: small | Port: 8201
...
SERVICE_PORT        = config.get('port', 8201)
...
app = FastAPI(
    title="Notifications-Center-Frontend API",
    description="Frontend service | Port 8201 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "notifications-center-frontend", "type": "frontend",
...
async def health():
    return {
        "status": "healthy", "service": "notifications-center-frontend",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "notifications-center-frontend", "port": SERVICE_PORT,
...
async def root():
    ...
    return {"service": "notifications-center-frontend", "domain": "frontend",
            "endpoint": "/", "method": "GET",
            "status": "ok", "data": {},
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "notifications-center-frontend", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting notifications-center-frontend port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
