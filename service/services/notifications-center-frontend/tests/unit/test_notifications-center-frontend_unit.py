"""Unit tests for notifications-center-frontend.
Template: svc_frontend_003 | Port: 8201 | Tier: small
...
class TestNotificationsCenterFrontendUnit:
    """Unit tests — port 8201 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "notifications-center-frontend"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8201, f"Port mismatch: expected 8201 got {d['port']}"
    ...
    def test_endpoint_get_root(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "notifications-center-frontend", \
