"""Integration tests for traditional_dashboard_orchestrator.
Port: 8158 | Tier: medium | Namespace: jtp
...
class TestTraditionalDashboardOrchestratorIntegration:
    ...
    def test_domain_endpoint_analytics_dashboard(self, base_url):
        """Integration: GET /analytics/dashboard responds correctly."""
        try:
            r = httpx.get(f"{base_url}/analytics/dashboard", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /analytics/dashboard returned {r.status_code}"
            data = r.json()
    ...
    def test_domain_endpoint_analytics_metrics(self, base_url):
        """Integration: GET /analytics/metrics responds correctly."""
        try:
            r = httpx.get(f"{base_url}/analytics/metrics", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /analytics/metrics returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
    ...
    def test_domain_endpoint_analytics_track(self, base_url):
        """Integration: POST /analytics/track responds correctly."""
        try:
            r = httpx.post(f"{base_url}/analytics/track", json={}, timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /analytics/track returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
