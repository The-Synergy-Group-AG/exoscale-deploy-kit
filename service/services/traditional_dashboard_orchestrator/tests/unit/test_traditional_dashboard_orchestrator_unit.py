"""Unit tests for traditional_dashboard_orchestrator.
Template: svc_analytics_009 | Port: 8158 | Tier: medium
...
class TestTraditionalDashboardOrchestratorUnit:
    """Unit tests — port 8158 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "traditional_dashboard_orchestrator"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8158, f"Port mismatch: expected 8158 got {d['port']}"
    ...
    def test_endpoint_get_analytics_dashboard(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "traditional_dashboard_orchestrator", \
