"""Unit tests for affiliate_manager_service.
Template: svc_workflow_012 | Port: 8276 | Tier: medium
...
class TestAffiliateManagerServiceUnit:
    """Unit tests — port 8276 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "affiliate_manager_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8276, f"Port mismatch: expected 8276 got {d['port']}"
    ...
    def test_endpoint_get_workflows(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "affiliate_manager_service", \
