"""Unit tests for autonomous_security_test_suite.
Template: svc_monitoring_016 | Port: 8151 | Tier: large
...
class TestAutonomousSecurityTestSuiteUnit:
    """Unit tests — port 8151 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "autonomous_security_test_suite"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8151, f"Port mismatch: expected 8151 got {d['port']}"
    ...
    def test_endpoint_get_security_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "autonomous_security_test_suite", \
