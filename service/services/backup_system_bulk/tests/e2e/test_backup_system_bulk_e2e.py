"""E2E tests for backup_system_bulk via gateway.
Port: 8095 | NS: jtp
...
class TestBackupSystemBulkE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/backup_system_bulk/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/backup_system_bulk/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_backup_status(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/backup_system_bulk/backup/status", timeout=30.0)
    ...
    def test_gateway_endpoint_backup_create(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/backup_system_bulk/backup/create", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_backup_id(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/backup_system_bulk/backup/test-id", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_restore(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/backup_system_bulk/restore", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_recovery_status(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/backup_system_bulk/recovery/status", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_backup_id(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/backup_system_bulk/backup/test-id", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Gateway endpoint /backup/{id} returned {r.status_code}"
            assert r.json() is not None
        except httpx.ConnectError:
