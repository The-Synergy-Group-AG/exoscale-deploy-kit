"""Unit tests for backup_system_bulk.
Template: svc_health_017 | Port: 8095 | Tier: small
...
class TestBackupSystemBulkUnit:
    """Unit tests — port 8095 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "backup_system_bulk"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8095, f"Port mismatch: expected 8095 got {d['port']}"
    ...
    def test_endpoint_get_backup_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "backup_system_bulk", \
