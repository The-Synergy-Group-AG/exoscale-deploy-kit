"""Unit tests for harmony_optimization_system_category.
Template: svc_health_017 | Port: 8160 | Tier: large
...
class TestHarmonyOptimizationSystemCategoryUnit:
    """Unit tests — port 8160 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "harmony_optimization_system_category"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8160, f"Port mismatch: expected 8160 got {d['port']}"
    ...
    def test_endpoint_get_biological_harmony(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "harmony_optimization_system_category", \
