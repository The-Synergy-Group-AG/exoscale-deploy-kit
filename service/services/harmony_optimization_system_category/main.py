"""
Harmony Optimization System Category — Biological Service

Template: svc_health_017
Domain: biological | Tier: large | Port: 8160
...
SERVICE_PORT        = config.get('port', 8160)
...
app = FastAPI(
    title="Harmony Optimization System Category API",
    description="Biological service | Port 8160 | Tier: large | NS: jtp",
...
async def root():
    return {
        "service": "harmony_optimization_system_category", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "harmony_optimization_system_category",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "harmony_optimization_system_category", "port": SERVICE_PORT,
...
async def biological_harmony():
    ...
    return {"service": "harmony_optimization_system_category", "domain": "biological",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "harmony_optimization_system_category", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting harmony_optimization_system_category port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
