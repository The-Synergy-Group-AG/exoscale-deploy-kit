"""Unit tests for advanced_analytics_bi_service.
Template: svc_analytics_009 | Port: 8164 | Tier: medium
...
class TestAdvancedAnalyticsBiServiceUnit:
    """Unit tests — port 8164 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "advanced_analytics_bi_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8164, f"Port mismatch: expected 8164 got {d['port']}"
    ...
    def test_endpoint_get_analytics_dashboard(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "advanced_analytics_bi_service", \
