"""Unit tests for cvs_service.
Template: svc_catalog_008 | Port: 8208 | Tier: medium
...
class TestCvsServiceUnit:
    """Unit tests — port 8208 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "cvs_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8208, f"Port mismatch: expected 8208 got {d['port']}"
    ...
    def test_endpoint_get_jobs(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "cvs_service", \
