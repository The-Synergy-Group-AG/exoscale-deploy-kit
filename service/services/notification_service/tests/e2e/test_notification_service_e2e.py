"""E2E tests for notification_service via gateway.
Port: 8260 | NS: jtp
...
class TestNotificationServiceE2E:
    ...
    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/notification_service/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/notification_service/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_notifications(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/notification_service/notifications", timeout=30.0)
    ...
    def test_gateway_endpoint_notifications_send(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/notification_service/notifications/send", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_notifications_id_read(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/notification_service/notifications/test-id/read", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_notifications_id(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/notification_service/notifications/test-id", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_notifications_summary(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/notification_service/notifications/summary", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_notifications_bulk(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/notification_service/notifications/bulk", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
