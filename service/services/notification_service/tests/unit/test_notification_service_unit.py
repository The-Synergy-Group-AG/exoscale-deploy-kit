...
Template: svc_notification_004 | Port: 8260 | Tier: small
...
class TestNotificationServiceUnit:
    """Unit tests — port 8260 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "notification_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8260, f"Port mismatch: expected 8260 got {d['port']}"
    ...
    def test_endpoint_get_notifications(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "notification_service", \
