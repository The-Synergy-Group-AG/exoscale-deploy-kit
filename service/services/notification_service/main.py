...
Domain: notification | Tier: small | Port: 8260
...
SERVICE_PORT        = config.get('port', 8260)
...
app = FastAPI(
    ...
    description="Notification service | Port 8260 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "notification_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "notification_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "notification_service", "port": SERVICE_PORT,
...
async def notifications():
    ...
    return {"service": "notification_service", "domain": "notification",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "notification_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting notification_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
