"""Unit tests for metabolic_system_orchestrator_bulk.
Template: svc_health_017 | Port: 8094 | Tier: large
...
class TestMetabolicSystemOrchestratorBulkUnit:
    """Unit tests — port 8094 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "metabolic_system_orchestrator_bulk"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8094, f"Port mismatch: expected 8094 got {d['port']}"
    ...
    def test_endpoint_get_biological_harmony(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "metabolic_system_orchestrator_bulk", \
