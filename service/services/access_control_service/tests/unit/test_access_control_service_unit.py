"""Unit tests for access_control_service.
Template: svc_monitoring_016 | Port: 8251 | Tier: large
...
class TestAccessControlServiceUnit:
    """Unit tests — port 8251 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "access_control_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8251, f"Port mismatch: expected 8251 got {d['port']}"
    ...
    def test_endpoint_get_security_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "access_control_service", \
