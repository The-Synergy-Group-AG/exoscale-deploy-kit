"""E2E tests for access_control_service via gateway.
Port: 8251 | NS: jtp
...
class TestAccessControlServiceE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/access_control_service/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/access_control_service/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_security_status(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/access_control_service/security/status", timeout=30.0)
    ...
    def test_gateway_endpoint_security_scan(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/access_control_service/security/scan", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_threats(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/access_control_service/threats", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_policies_id(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/access_control_service/policies/test-id", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_alerts(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/access_control_service/alerts", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_incidents_report(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/access_control_service/incidents/report", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
