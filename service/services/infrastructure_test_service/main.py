"""
Infrastructure Test Service — Monitoring Service

Template: svc_monitoring_016
Domain: monitoring | Tier: small | Port: 8117
...
SERVICE_PORT        = config.get('port', 8117)
...
app = FastAPI(
    title="Infrastructure Test Service API",
    description="Monitoring service | Port 8117 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "infrastructure_test_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "infrastructure_test_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "infrastructure_test_service", "port": SERVICE_PORT,
...
async def status():
    ...
    return {"service": "infrastructure_test_service", "domain": "monitoring",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "infrastructure_test_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting infrastructure_test_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
