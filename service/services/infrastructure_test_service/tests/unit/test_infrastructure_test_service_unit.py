"""Unit tests for infrastructure_test_service.
Template: svc_monitoring_016 | Port: 8117 | Tier: small
...
class TestInfrastructureTestServiceUnit:
    """Unit tests — port 8117 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "infrastructure_test_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8117, f"Port mismatch: expected 8117 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "infrastructure_test_service", \
