"""Unit tests for form_management_system.
Template: svc_workflow_012 | Port: 8259 | Tier: medium
...
class TestFormManagementSystemUnit:
    """Unit tests — port 8259 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "form_management_system"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8259, f"Port mismatch: expected 8259 got {d['port']}"
    ...
    def test_endpoint_get_workflows(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "form_management_system", \
