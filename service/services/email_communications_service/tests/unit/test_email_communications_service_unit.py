"""Unit tests for email_communications_service.
Template: svc_notification_004 | Port: 8195 | Tier: small
...
class TestEmailCommunicationsServiceUnit:
    """Unit tests — port 8195 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "email_communications_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8195, f"Port mismatch: expected 8195 got {d['port']}"
    ...
    def test_endpoint_get_notifications(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "email_communications_service", \
