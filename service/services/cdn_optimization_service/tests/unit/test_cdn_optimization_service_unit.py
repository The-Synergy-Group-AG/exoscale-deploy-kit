"""Unit tests for cdn_optimization_service.
Template: svc_workflow_012 | Port: 8303 | Tier: medium
...
class TestCdnOptimizationServiceUnit:
    """Unit tests — port 8303 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "cdn_optimization_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8303, f"Port mismatch: expected 8303 got {d['port']}"
    ...
    def test_endpoint_get_workflows(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "cdn_optimization_service", \
