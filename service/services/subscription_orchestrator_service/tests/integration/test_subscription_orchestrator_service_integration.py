"""Integration tests for subscription_orchestrator_service.
Port: 8223 | Tier: medium | Namespace: jtp
...
class TestSubscriptionOrchestratorServiceIntegration:
    ...
    def test_domain_endpoint_payments(self, base_url):
        """Integration: GET /payments responds correctly."""
        try:
            r = httpx.get(f"{base_url}/payments", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /payments returned {r.status_code}"
            data = r.json()
    ...
    def test_domain_endpoint_payments_process(self, base_url):
        """Integration: POST /payments/process responds correctly."""
        try:
            r = httpx.post(f"{base_url}/payments/process", json={}, timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /payments/process returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
    ...
    def test_domain_endpoint_payments_id(self, base_url):
        """Integration: GET /payments/{id} responds correctly."""
        try:
            r = httpx.get(f"{base_url}/payments/test-id", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /payments/{id} returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
