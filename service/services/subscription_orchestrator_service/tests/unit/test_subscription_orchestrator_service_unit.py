"""Unit tests for subscription_orchestrator_service.
Template: svc_payment_002 | Port: 8223 | Tier: medium
...
class TestSubscriptionOrchestratorServiceUnit:
    """Unit tests — port 8223 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "subscription_orchestrator_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8223, f"Port mismatch: expected 8223 got {d['port']}"
    ...
    def test_endpoint_get_payments(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "subscription_orchestrator_service", \
