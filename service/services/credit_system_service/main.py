"""
Credit System Service — Payment Service

Template: svc_payment_002
Domain: payment | Tier: medium | Port: 8217
...
SERVICE_PORT        = config.get('port', 8217)
...
app = FastAPI(
    title="Credit System Service API",
    description="Payment service | Port 8217 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "credit_system_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "credit_system_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "credit_system_service", "port": SERVICE_PORT,
...
async def payments():
    ...
    return {"service": "credit_system_service", "domain": "payment",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "credit_system_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting credit_system_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
