"""Unit tests for credit_system_service.
Template: svc_payment_002 | Port: 8217 | Tier: medium
...
class TestCreditSystemServiceUnit:
    """Unit tests — port 8217 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "credit_system_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8217, f"Port mismatch: expected 8217 got {d['port']}"
    ...
    def test_endpoint_get_payments(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "credit_system_service", \
