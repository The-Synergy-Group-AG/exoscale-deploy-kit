...
Template: svc_notification_004 | Port: 8154 | Tier: small
...
class TestEmailIntegrationServiceUnit:
    """Unit tests — port 8154 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "email_integration_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8154, f"Port mismatch: expected 8154 got {d['port']}"
    ...
    def test_endpoint_get_notifications(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "email_integration_service", \
