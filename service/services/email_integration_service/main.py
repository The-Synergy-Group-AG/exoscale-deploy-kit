"""
Email Integration Service — Notification Service

Template: svc_notification_004
Domain: notification | Tier: small | Port: 8154
...
SERVICE_PORT        = config.get('port', 8154)
...
app = FastAPI(
    title="Email Integration Service API",
    description="Notification service | Port 8154 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "email_integration_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "email_integration_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "email_integration_service", "port": SERVICE_PORT,
...
async def notifications():
    ...
    return {"service": "email_integration_service", "domain": "notification",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "email_integration_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting email_integration_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
