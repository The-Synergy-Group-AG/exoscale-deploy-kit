"""Unit tests for personalization_ai_adaptor.
Template: svc_analytics_009 | Port: 8087 | Tier: large
...
class TestPersonalizationAiAdaptorUnit:
    """Unit tests — port 8087 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "personalization_ai_adaptor"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8087, f"Port mismatch: expected 8087 got {d['port']}"
    ...
    def test_endpoint_post_ai_process(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "personalization_ai_adaptor", \
