"""Unit tests for resource_manager_bulk.
Template: svc_workflow_012 | Port: 8199 | Tier: medium
...
class TestResourceManagerBulkUnit:
    """Unit tests — port 8199 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "resource_manager_bulk"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8199, f"Port mismatch: expected 8199 got {d['port']}"
    ...
    def test_endpoint_get_workflows(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "resource_manager_bulk", \
