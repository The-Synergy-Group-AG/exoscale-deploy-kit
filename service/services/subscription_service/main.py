...
Domain: payment | Tier: medium | Port: 8204
...
SERVICE_PORT        = config.get('port', 8204)
...
app = FastAPI(
    ...
    description="Payment service | Port 8204 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "subscription_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "subscription_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "subscription_service", "port": SERVICE_PORT,
...
async def payments():
    ...
    return {"service": "subscription_service", "domain": "payment",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "subscription_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting subscription_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
