...
Template: svc_payment_002 | Port: 8204 | Tier: medium
...
class TestSubscriptionServiceUnit:
    """Unit tests — port 8204 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "subscription_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8204, f"Port mismatch: expected 8204 got {d['port']}"
    ...
    def test_endpoint_get_payments(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "subscription_service", \
