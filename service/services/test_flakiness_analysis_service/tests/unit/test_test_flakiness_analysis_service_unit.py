"""Unit tests for test_flakiness_analysis_service.
Template: svc_monitoring_016 | Port: 8278 | Tier: small
...
class TestTestFlakinessAnalysisServiceUnit:
    """Unit tests — port 8278 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "test_flakiness_analysis_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8278, f"Port mismatch: expected 8278 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "test_flakiness_analysis_service", \
