"""Unit tests for performance_optimization_layer.
Template: svc_monitoring_016 | Port: 8207 | Tier: small
...
class TestPerformanceOptimizationLayerUnit:
    """Unit tests — port 8207 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "performance_optimization_layer"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8207, f"Port mismatch: expected 8207 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "performance_optimization_layer", \
