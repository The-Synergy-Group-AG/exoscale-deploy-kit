"""
Secure Messaging Gateway Communication — Security Service

Template: svc_monitoring_016
Domain: security | Tier: large | Port: 8226
...
SERVICE_PORT        = config.get('port', 8226)
...
app = FastAPI(
    title="Secure Messaging Gateway Communication API",
    description="Security service | Port 8226 | Tier: large | NS: jtp",
...
async def root():
    return {
        "service": "secure_messaging_gateway_communication", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "secure_messaging_gateway_communication",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "secure_messaging_gateway_communication", "port": SERVICE_PORT,
...
async def security_status():
    ...
    return {"service": "secure_messaging_gateway_communication", "domain": "security",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "secure_messaging_gateway_communication", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting secure_messaging_gateway_communication port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
