"""Unit tests for secure_messaging_gateway_communication.
Template: svc_monitoring_016 | Port: 8226 | Tier: large
...
class TestSecureMessagingGatewayCommunicationUnit:
    """Unit tests — port 8226 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "secure_messaging_gateway_communication"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8226, f"Port mismatch: expected 8226 got {d['port']}"
    ...
    def test_endpoint_get_security_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "secure_messaging_gateway_communication", \
