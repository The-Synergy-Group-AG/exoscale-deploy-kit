"""Unit tests for audit_logging_service.
Template: svc_audit_014 | Port: 8097 | Tier: medium
...
class TestAuditLoggingServiceUnit:
    """Unit tests — port 8097 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "audit_logging_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8097, f"Port mismatch: expected 8097 got {d['port']}"
    ...
    def test_endpoint_get_compliance_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "audit_logging_service", \
