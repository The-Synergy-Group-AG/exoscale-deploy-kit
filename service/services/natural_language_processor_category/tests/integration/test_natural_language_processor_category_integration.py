"""Integration tests for natural_language_processor_category.
Port: 8107 | Tier: medium | Namespace: jtp
...
class TestNaturalLanguageProcessorCategoryIntegration:
    ...
    def test_domain_endpoint_auth_status(self, base_url):
        """Integration: GET /auth/status responds correctly."""
        try:
            r = httpx.get(f"{base_url}/auth/status", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /auth/status returned {r.status_code}"
            data = r.json()
    ...
    def test_domain_endpoint_auth_login(self, base_url):
        """Integration: POST /auth/login responds correctly."""
        try:
            r = httpx.post(f"{base_url}/auth/login", json={}, timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /auth/login returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
    ...
    def test_domain_endpoint_auth_logout(self, base_url):
        """Integration: POST /auth/logout responds correctly."""
        try:
            r = httpx.post(f"{base_url}/auth/logout", json={}, timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /auth/logout returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
