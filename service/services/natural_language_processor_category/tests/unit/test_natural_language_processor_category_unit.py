"""Unit tests for natural_language_processor_category.
Template: svc_authentication_001 | Port: 8107 | Tier: medium
...
class TestNaturalLanguageProcessorCategoryUnit:
    """Unit tests — port 8107 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "natural_language_processor_category"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8107, f"Port mismatch: expected 8107 got {d['port']}"
    ...
    def test_endpoint_get_auth_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "natural_language_processor_category", \
