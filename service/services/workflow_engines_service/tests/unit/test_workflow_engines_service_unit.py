"""Unit tests for workflow_engines_service.
Template: svc_workflow_012 | Port: 8184 | Tier: medium
...
class TestWorkflowEnginesServiceUnit:
    """Unit tests — port 8184 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "workflow_engines_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8184, f"Port mismatch: expected 8184 got {d['port']}"
    ...
    def test_endpoint_get_workflows(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "workflow_engines_service", \
