"""Unit tests for test_api_rate_limiting_validation.
Template: svc_monitoring_016 | Port: 8180 | Tier: small
...
class TestTestApiRateLimitingValidationUnit:
    """Unit tests — port 8180 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "test_api_rate_limiting_validation"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8180, f"Port mismatch: expected 8180 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "test_api_rate_limiting_validation", \
