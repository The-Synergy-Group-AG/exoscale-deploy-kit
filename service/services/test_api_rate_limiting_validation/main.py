"""
Test Api Rate Limiting Validation — Monitoring Service

Template: svc_monitoring_016
Domain: monitoring | Tier: small | Port: 8180
...
SERVICE_PORT        = config.get('port', 8180)
...
app = FastAPI(
    title="Test Api Rate Limiting Validation API",
    description="Monitoring service | Port 8180 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "test_api_rate_limiting_validation", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "test_api_rate_limiting_validation",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "test_api_rate_limiting_validation", "port": SERVICE_PORT,
...
async def status():
    ...
    return {"service": "test_api_rate_limiting_validation", "domain": "monitoring",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "test_api_rate_limiting_validation", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting test_api_rate_limiting_validation port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
