"""Unit tests for real_time_data_refresher.
Template: svc_analytics_009 | Port: 8126 | Tier: large
...
class TestRealTimeDataRefresherUnit:
    """Unit tests — port 8126 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "real_time_data_refresher"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8126, f"Port mismatch: expected 8126 got {d['port']}"
    ...
    def test_endpoint_post_ai_process(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "real_time_data_refresher", \
