"""Unit tests for consciousness_memory_system_other.
Template: svc_health_017 | Port: 8120 | Tier: large
...
class TestConsciousnessMemorySystemOtherUnit:
    """Unit tests — port 8120 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "consciousness_memory_system_other"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8120, f"Port mismatch: expected 8120 got {d['port']}"
    ...
    def test_endpoint_get_biological_harmony(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "consciousness_memory_system_other", \
