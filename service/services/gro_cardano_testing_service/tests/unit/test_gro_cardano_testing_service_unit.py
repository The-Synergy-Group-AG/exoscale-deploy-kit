"""Unit tests for gro_cardano_testing_service.
Template: svc_catalog_008 | Port: 8101 | Tier: small
...
class TestGroCardanoTestingServiceUnit:
    """Unit tests — port 8101 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "gro_cardano_testing_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8101, f"Port mismatch: expected 8101 got {d['port']}"
    ...
    def test_endpoint_get_data(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "gro_cardano_testing_service", \
