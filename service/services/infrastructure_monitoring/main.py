...
Domain: monitoring | Tier: small | Port: 8210
...
SERVICE_PORT        = config.get('port', 8210)
...
app = FastAPI(
    ...
    description="Monitoring service | Port 8210 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "infrastructure_monitoring", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "infrastructure_monitoring",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "infrastructure_monitoring", "port": SERVICE_PORT,
...
async def status():
    ...
    return {"service": "infrastructure_monitoring", "domain": "monitoring",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "infrastructure_monitoring", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting infrastructure_monitoring port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
