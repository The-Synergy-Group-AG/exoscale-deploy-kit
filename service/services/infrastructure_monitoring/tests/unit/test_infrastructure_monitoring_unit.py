"""Unit tests for infrastructure_monitoring.
Template: svc_monitoring_016 | Port: 8210 | Tier: small
...
class TestInfrastructureMonitoringUnit:
    """Unit tests — port 8210 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "infrastructure_monitoring"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8210, f"Port mismatch: expected 8210 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "infrastructure_monitoring", \
