"""Unit tests for gamification-frontend.
Template: svc_frontend_003 | Port: 8156 | Tier: small
...
class TestGamificationFrontendUnit:
    """Unit tests — port 8156 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "gamification-frontend"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8156, f"Port mismatch: expected 8156 got {d['port']}"
    ...
    def test_endpoint_get_root(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "gamification-frontend", \
