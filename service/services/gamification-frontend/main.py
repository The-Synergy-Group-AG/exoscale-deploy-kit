"""
Gamification-Frontend — Frontend Service

Template: svc_frontend_003
Domain: frontend | Tier: small | Port: 8156
...
SERVICE_PORT        = config.get('port', 8156)
...
app = FastAPI(
    title="Gamification-Frontend API",
    description="Frontend service | Port 8156 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "gamification-frontend", "type": "frontend",
...
async def health():
    return {
        "status": "healthy", "service": "gamification-frontend",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "gamification-frontend", "port": SERVICE_PORT,
...
async def root():
    ...
    return {"service": "gamification-frontend", "domain": "frontend",
            "endpoint": "/", "method": "GET",
            "status": "ok", "data": {},
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "gamification-frontend", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting gamification-frontend port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
