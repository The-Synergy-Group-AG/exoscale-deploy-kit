"""Unit tests for environment_provisioning_test_service.
Template: svc_monitoring_016 | Port: 8269 | Tier: small
...
class TestEnvironmentProvisioningTestServiceUnit:
    """Unit tests — port 8269 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "environment_provisioning_test_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8269, f"Port mismatch: expected 8269 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "environment_provisioning_test_service", \
