"""Unit tests for skeletal_system.
Template: svc_health_017 | Port: 8222 | Tier: large
...
class TestSkeletalSystemUnit:
    """Unit tests — port 8222 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "skeletal_system"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8222, f"Port mismatch: expected 8222 got {d['port']}"
    ...
    def test_endpoint_get_biological_harmony(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "skeletal_system", \
