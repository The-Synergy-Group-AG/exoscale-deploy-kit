"""Unit tests for neural_network_processor_ai.
Template: svc_authentication_001 | Port: 8219 | Tier: medium
...
class TestNeuralNetworkProcessorAiUnit:
    """Unit tests — port 8219 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "neural_network_processor_ai"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8219, f"Port mismatch: expected 8219 got {d['port']}"
    ...
    def test_endpoint_get_auth_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "neural_network_processor_ai", \
