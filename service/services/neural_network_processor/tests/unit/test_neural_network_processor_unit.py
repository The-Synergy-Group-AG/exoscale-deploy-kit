"""Unit tests for neural_network_processor.
Template: svc_authentication_001 | Port: 8214 | Tier: medium
...
class TestNeuralNetworkProcessorUnit:
    """Unit tests — port 8214 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "neural_network_processor"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8214, f"Port mismatch: expected 8214 got {d['port']}"
    ...
    def test_endpoint_get_auth_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "neural_network_processor", \
