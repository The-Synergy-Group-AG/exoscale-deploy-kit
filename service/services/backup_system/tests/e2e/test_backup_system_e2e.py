"""E2E tests for backup_system via gateway.
Port: 8171 | NS: jtp
...
class TestBackupSystemE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/backup_system/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/backup_system/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_backup_status(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/backup_system/backup/status", timeout=30.0)
    ...
    def test_gateway_endpoint_backup_create(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/backup_system/backup/create", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_backup_id(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/backup_system/backup/test-id", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_restore(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/backup_system/restore", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_recovery_status(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/backup_system/recovery/status", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_backup_id(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/backup_system/backup/test-id", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Gateway endpoint /backup/{id} returned {r.status_code}"
            assert r.json() is not None
        except httpx.ConnectError:
