"""
Backup System — Backup Service

Template: svc_health_017
Domain: backup | Tier: small | Port: 8171
...
SERVICE_PORT        = config.get('port', 8171)
...
app = FastAPI(
    ...
    description="Backup service | Port 8171 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "backup_system", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "backup_system",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "backup_system", "port": SERVICE_PORT,
...
async def backup_status():
    ...
    return {"service": "backup_system", "domain": "backup",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "backup_system", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting backup_system port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
