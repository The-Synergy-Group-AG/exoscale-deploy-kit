"""Unit tests for computer_vision_service.
Template: svc_analytics_009 | Port: 8137 | Tier: large
...
class TestComputerVisionServiceUnit:
    """Unit tests — port 8137 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "computer_vision_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8137, f"Port mismatch: expected 8137 got {d['port']}"
    ...
    def test_endpoint_post_ai_process(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "computer_vision_service", \
