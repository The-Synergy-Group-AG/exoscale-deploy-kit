"""Unit tests for document_management_service.
Template: svc_catalog_008 | Port: 8246 | Tier: medium
...
class TestDocumentManagementServiceUnit:
    """Unit tests — port 8246 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "document_management_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8246, f"Port mismatch: expected 8246 got {d['port']}"
    ...
    def test_endpoint_get_documents(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "document_management_service", \
