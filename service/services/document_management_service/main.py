...
Domain: document | Tier: medium | Port: 8246
...
SERVICE_PORT        = config.get('port', 8246)
...
app = FastAPI(
    ...
    description="Document service | Port 8246 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "document_management_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "document_management_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "document_management_service", "port": SERVICE_PORT,
...
async def documents():
    ...
    return {"service": "document_management_service", "domain": "document",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "document_management_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting document_management_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
