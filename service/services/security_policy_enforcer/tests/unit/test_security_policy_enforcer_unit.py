"""Unit tests for security_policy_enforcer.
Template: svc_monitoring_016 | Port: 8106 | Tier: large
...
class TestSecurityPolicyEnforcerUnit:
    """Unit tests — port 8106 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "security_policy_enforcer"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8106, f"Port mismatch: expected 8106 got {d['port']}"
    ...
    def test_endpoint_get_security_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "security_policy_enforcer", \
