"""Integration tests for machine_learning_pipelines_service.
Port: 8257 | Tier: large | Namespace: jtp
...
class TestMachineLearningPipelinesServiceIntegration:
    ...
    def test_domain_endpoint_ai_process(self, base_url):
        """Integration: POST /ai/process responds correctly."""
        try:
            r = httpx.post(f"{base_url}/ai/process", json={}, timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /ai/process returned {r.status_code}"
            data = r.json()
    ...
    def test_domain_endpoint_models(self, base_url):
        """Integration: GET /models responds correctly."""
        try:
            r = httpx.get(f"{base_url}/models", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /models returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
    ...
    def test_domain_endpoint_models_predict(self, base_url):
        """Integration: POST /models/predict responds correctly."""
        try:
            r = httpx.post(f"{base_url}/models/predict", json={}, timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /models/predict returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
