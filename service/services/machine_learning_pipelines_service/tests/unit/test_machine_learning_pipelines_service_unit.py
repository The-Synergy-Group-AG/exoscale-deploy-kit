"""Unit tests for machine_learning_pipelines_service.
Template: svc_analytics_009 | Port: 8257 | Tier: large
...
class TestMachineLearningPipelinesServiceUnit:
    """Unit tests — port 8257 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "machine_learning_pipelines_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8257, f"Port mismatch: expected 8257 got {d['port']}"
    ...
    def test_endpoint_post_ai_process(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "machine_learning_pipelines_service", \
