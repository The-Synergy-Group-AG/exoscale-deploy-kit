"""
Navigation System — Workflow Service

Template: svc_workflow_012
Domain: workflow | Tier: medium | Port: 8241
...
SERVICE_PORT        = config.get('port', 8241)
...
app = FastAPI(
    ...
    description="Workflow service | Port 8241 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "navigation_system", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "navigation_system",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "navigation_system", "port": SERVICE_PORT,
...
async def workflows():
    ...
    return {"service": "navigation_system", "domain": "workflow",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "navigation_system", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting navigation_system port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
