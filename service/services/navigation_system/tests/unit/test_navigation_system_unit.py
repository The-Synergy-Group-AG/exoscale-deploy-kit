"""Unit tests for navigation_system.
Template: svc_workflow_012 | Port: 8241 | Tier: medium
...
class TestNavigationSystemUnit:
    """Unit tests — port 8241 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "navigation_system"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8241, f"Port mismatch: expected 8241 got {d['port']}"
    ...
    def test_endpoint_get_workflows(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "navigation_system", \
