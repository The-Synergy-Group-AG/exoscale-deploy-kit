"""
Referral Tracking Service — Workflow Service

Template: svc_workflow_012
Domain: workflow | Tier: medium | Port: 8114
...
SERVICE_PORT        = config.get('port', 8114)
...
app = FastAPI(
    title="Referral Tracking Service API",
    description="Workflow service | Port 8114 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "referral_tracking_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "referral_tracking_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "referral_tracking_service", "port": SERVICE_PORT,
...
async def workflows():
    ...
    return {"service": "referral_tracking_service", "domain": "workflow",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "referral_tracking_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting referral_tracking_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
