"""Unit tests for referral_tracking_service.
Template: svc_workflow_012 | Port: 8114 | Tier: medium
...
class TestReferralTrackingServiceUnit:
    """Unit tests — port 8114 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "referral_tracking_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8114, f"Port mismatch: expected 8114 got {d['port']}"
    ...
    def test_endpoint_get_workflows(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "referral_tracking_service", \
