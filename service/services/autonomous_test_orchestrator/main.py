"""
Autonomous Test Orchestrator — Monitoring Service

Template: svc_monitoring_016
Domain: monitoring | Tier: small | Port: 8294
...
SERVICE_PORT        = config.get('port', 8294)
...
app = FastAPI(
    title="Autonomous Test Orchestrator API",
    description="Monitoring service | Port 8294 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "autonomous_test_orchestrator", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "autonomous_test_orchestrator",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "autonomous_test_orchestrator", "port": SERVICE_PORT,
...
async def status():
    ...
    return {"service": "autonomous_test_orchestrator", "domain": "monitoring",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "autonomous_test_orchestrator", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting autonomous_test_orchestrator port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
