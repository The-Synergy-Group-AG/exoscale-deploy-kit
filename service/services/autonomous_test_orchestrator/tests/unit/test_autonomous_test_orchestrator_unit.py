"""Unit tests for autonomous_test_orchestrator.
Template: svc_monitoring_016 | Port: 8294 | Tier: small
...
class TestAutonomousTestOrchestratorUnit:
    """Unit tests — port 8294 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "autonomous_test_orchestrator"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8294, f"Port mismatch: expected 8294 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "autonomous_test_orchestrator", \
