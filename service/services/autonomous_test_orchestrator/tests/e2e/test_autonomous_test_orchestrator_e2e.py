"""E2E tests for autonomous_test_orchestrator via gateway.
Port: 8294 | NS: jtp
...
class TestAutonomousTestOrchestratorE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/autonomous_test_orchestrator/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/autonomous_test_orchestrator/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_status(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/autonomous_test_orchestrator/status", timeout=30.0)
    ...
    def test_gateway_endpoint_metrics_live(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/autonomous_test_orchestrator/metrics/live", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_alerts_create(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/autonomous_test_orchestrator/alerts/create", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_logs(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/autonomous_test_orchestrator/logs", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_performance(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/autonomous_test_orchestrator/performance", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_alerts_id(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/autonomous_test_orchestrator/alerts/test-id", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
