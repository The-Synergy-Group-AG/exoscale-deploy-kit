"""Unit tests for neural_consciousness_amplifier.
Template: svc_analytics_009 | Port: 8155 | Tier: large
...
class TestNeuralConsciousnessAmplifierUnit:
    """Unit tests — port 8155 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "neural_consciousness_amplifier"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8155, f"Port mismatch: expected 8155 got {d['port']}"
    ...
    def test_endpoint_post_ai_process(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "neural_consciousness_amplifier", \
