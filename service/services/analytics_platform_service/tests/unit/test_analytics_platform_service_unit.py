"""Unit tests for analytics_platform_service.
Template: svc_analytics_009 | Port: 8289 | Tier: medium
...
class TestAnalyticsPlatformServiceUnit:
    """Unit tests — port 8289 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "analytics_platform_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8289, f"Port mismatch: expected 8289 got {d['port']}"
    ...
    def test_endpoint_get_analytics_dashboard(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "analytics_platform_service", \
