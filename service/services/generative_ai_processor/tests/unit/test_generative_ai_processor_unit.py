"""Unit tests for generative_ai_processor.
Template: svc_authentication_001 | Port: 8261 | Tier: medium
...
class TestGenerativeAiProcessorUnit:
    """Unit tests — port 8261 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "generative_ai_processor"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8261, f"Port mismatch: expected 8261 got {d['port']}"
    ...
    def test_endpoint_get_auth_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "generative_ai_processor", \
