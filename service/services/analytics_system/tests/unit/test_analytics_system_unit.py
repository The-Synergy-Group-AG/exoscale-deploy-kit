"""Unit tests for analytics_system.
Template: svc_analytics_009 | Port: 8182 | Tier: medium
...
class TestAnalyticsSystemUnit:
    """Unit tests — port 8182 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "analytics_system"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8182, f"Port mismatch: expected 8182 got {d['port']}"
    ...
    def test_endpoint_get_analytics_dashboard(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "analytics_system", \
