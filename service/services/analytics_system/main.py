...
Domain: analytics | Tier: medium | Port: 8182
...
SERVICE_PORT        = config.get('port', 8182)
...
app = FastAPI(
    ...
    description="Analytics service | Port 8182 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "analytics_system", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "analytics_system",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "analytics_system", "port": SERVICE_PORT,
...
async def analytics_dashboard():
    ...
    return {"service": "analytics_system", "domain": "analytics",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "analytics_system", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting analytics_system port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
