"""E2E tests for neural_network_validation_service via gateway.
Port: 8287 | NS: jtp
...
class TestNeuralNetworkValidationServiceE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/neural_network_validation_service/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/neural_network_validation_service/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_status(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/neural_network_validation_service/status", timeout=30.0)
    ...
    def test_gateway_endpoint_metrics_live(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/neural_network_validation_service/metrics/live", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_alerts_create(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/neural_network_validation_service/alerts/create", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_logs(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/neural_network_validation_service/logs", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_performance(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/neural_network_validation_service/performance", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_alerts_id(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/neural_network_validation_service/alerts/test-id", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
