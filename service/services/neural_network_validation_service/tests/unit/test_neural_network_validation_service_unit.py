"""Unit tests for neural_network_validation_service.
Template: svc_monitoring_016 | Port: 8287 | Tier: small
...
class TestNeuralNetworkValidationServiceUnit:
    """Unit tests — port 8287 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "neural_network_validation_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8287, f"Port mismatch: expected 8287 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "neural_network_validation_service", \
