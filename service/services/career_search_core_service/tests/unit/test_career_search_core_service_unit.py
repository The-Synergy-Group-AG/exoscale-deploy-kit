"""Unit tests for career_search_core_service.
Template: svc_catalog_008 | Port: 8262 | Tier: medium
...
class TestCareerSearchCoreServiceUnit:
    """Unit tests — port 8262 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "career_search_core_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8262, f"Port mismatch: expected 8262 got {d['port']}"
    ...
    def test_endpoint_get_jobs(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "career_search_core_service", \
