"""Unit tests for networking_service.
Template: svc_catalog_008 | Port: 8162 | Tier: small
...
class TestNetworkingServiceUnit:
    """Unit tests — port 8162 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "networking_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8162, f"Port mismatch: expected 8162 got {d['port']}"
    ...
    def test_endpoint_get_data(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "networking_service", \
