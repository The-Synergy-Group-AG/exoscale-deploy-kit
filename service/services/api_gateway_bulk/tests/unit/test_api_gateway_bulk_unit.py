"""Unit tests for api_gateway_bulk.
Template: svc_monitoring_016 | Port: 8169 | Tier: large
...
class TestApiGatewayBulkUnit:
    """Unit tests — port 8169 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "api_gateway_bulk"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8169, f"Port mismatch: expected 8169 got {d['port']}"
    ...
    def test_endpoint_get_security_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "api_gateway_bulk", \
