"""Unit tests for infrastructure_dashboard_bulk.
Template: svc_monitoring_016 | Port: 8086 | Tier: small
...
class TestInfrastructureDashboardBulkUnit:
    """Unit tests — port 8086 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "infrastructure_dashboard_bulk"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8086, f"Port mismatch: expected 8086 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "infrastructure_dashboard_bulk", \
