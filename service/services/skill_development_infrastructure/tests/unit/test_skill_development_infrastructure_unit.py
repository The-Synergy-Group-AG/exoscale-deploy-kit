"""Unit tests for skill_development_infrastructure.
Template: svc_catalog_008 | Port: 8240 | Tier: medium
...
class TestSkillDevelopmentInfrastructureUnit:
    """Unit tests — port 8240 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "skill_development_infrastructure"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8240, f"Port mismatch: expected 8240 got {d['port']}"
    ...
    def test_endpoint_get_jobs(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "skill_development_infrastructure", \
