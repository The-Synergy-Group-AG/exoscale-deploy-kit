"""
Skill Development Infrastructure — Career Service

Template: svc_catalog_008
Domain: career | Tier: medium | Port: 8240
...
SERVICE_PORT        = config.get('port', 8240)
...
app = FastAPI(
    title="Skill Development Infrastructure API",
    description="Career service | Port 8240 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "skill_development_infrastructure", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "skill_development_infrastructure",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "skill_development_infrastructure", "port": SERVICE_PORT,
...
async def jobs():
    ...
    return {"service": "skill_development_infrastructure", "domain": "career",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "skill_development_infrastructure", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting skill_development_infrastructure port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
