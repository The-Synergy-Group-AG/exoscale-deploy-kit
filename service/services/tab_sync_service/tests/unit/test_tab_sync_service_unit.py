"""Unit tests for tab_sync_service.
Template: svc_workflow_012 | Port: 8274 | Tier: medium
...
class TestTabSyncServiceUnit:
    """Unit tests — port 8274 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "tab_sync_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8274, f"Port mismatch: expected 8274 got {d['port']}"
    ...
    def test_endpoint_get_workflows(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "tab_sync_service", \
