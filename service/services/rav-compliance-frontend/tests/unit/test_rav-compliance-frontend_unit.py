"""Unit tests for rav-compliance-frontend.
Template: svc_frontend_003 | Port: 8228 | Tier: small
...
class TestRavComplianceFrontendUnit:
    """Unit tests — port 8228 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "rav-compliance-frontend"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8228, f"Port mismatch: expected 8228 got {d['port']}"
    ...
    def test_endpoint_get_root(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "rav-compliance-frontend", \
