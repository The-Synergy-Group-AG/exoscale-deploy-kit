"""Unit tests for reinforcement_learning_system.
Template: svc_analytics_009 | Port: 8253 | Tier: large
...
class TestReinforcementLearningSystemUnit:
    """Unit tests — port 8253 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "reinforcement_learning_system"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8253, f"Port mismatch: expected 8253 got {d['port']}"
    ...
    def test_endpoint_post_ai_process(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "reinforcement_learning_system", \
