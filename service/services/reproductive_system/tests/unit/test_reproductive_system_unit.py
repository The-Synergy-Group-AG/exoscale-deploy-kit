"""Unit tests for reproductive_system.
Template: svc_health_017 | Port: 8299 | Tier: large
...
class TestReproductiveSystemUnit:
    """Unit tests — port 8299 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "reproductive_system"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8299, f"Port mismatch: expected 8299 got {d['port']}"
    ...
    def test_endpoint_get_biological_harmony(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "reproductive_system", \
