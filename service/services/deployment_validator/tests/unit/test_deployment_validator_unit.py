"""Unit tests for deployment_validator.
Template: svc_monitoring_016 | Port: 8205 | Tier: small
...
class TestDeploymentValidatorUnit:
    """Unit tests — port 8205 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "deployment_validator"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8205, f"Port mismatch: expected 8205 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "deployment_validator", \
