"""
Deployment Validator — Monitoring Service

Template: svc_monitoring_016
Domain: monitoring | Tier: small | Port: 8205
...
SERVICE_PORT        = config.get('port', 8205)
...
app = FastAPI(
    title="Deployment Validator API",
    description="Monitoring service | Port 8205 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "deployment_validator", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "deployment_validator",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "deployment_validator", "port": SERVICE_PORT,
...
async def status():
    ...
    return {"service": "deployment_validator", "domain": "monitoring",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "deployment_validator", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting deployment_validator port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
