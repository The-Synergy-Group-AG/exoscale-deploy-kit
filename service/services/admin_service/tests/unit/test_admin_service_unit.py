...
Template: svc_user_management_006 | Port: 8088 | Tier: medium
...
class TestAdminServiceUnit:
    """Unit tests — port 8088 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "admin_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8088, f"Port mismatch: expected 8088 got {d['port']}"
    ...
    def test_endpoint_get_users(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "admin_service", \
