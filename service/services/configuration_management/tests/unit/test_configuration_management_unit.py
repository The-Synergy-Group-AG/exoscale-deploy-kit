"""Unit tests for configuration_management.
Template: svc_configuration_015 | Port: 8118 | Tier: small
...
class TestConfigurationManagementUnit:
    """Unit tests — port 8118 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "configuration_management"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8118, f"Port mismatch: expected 8118 got {d['port']}"
    ...
    def test_endpoint_get_config(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "configuration_management", \
