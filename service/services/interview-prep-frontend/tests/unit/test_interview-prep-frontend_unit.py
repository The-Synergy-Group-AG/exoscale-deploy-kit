"""Unit tests for interview-prep-frontend.
Template: svc_frontend_003 | Port: 8258 | Tier: small
...
class TestInterviewPrepFrontendUnit:
    """Unit tests — port 8258 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "interview-prep-frontend"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8258, f"Port mismatch: expected 8258 got {d['port']}"
    ...
    def test_endpoint_get_root(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "interview-prep-frontend", \
