"""
Interview-Prep-Frontend — Frontend Service

Template: svc_frontend_003
Domain: frontend | Tier: small | Port: 8258
...
SERVICE_PORT        = config.get('port', 8258)
...
app = FastAPI(
    title="Interview-Prep-Frontend API",
    description="Frontend service | Port 8258 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "interview-prep-frontend", "type": "frontend",
...
async def health():
    return {
        "status": "healthy", "service": "interview-prep-frontend",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "interview-prep-frontend", "port": SERVICE_PORT,
...
async def root():
    ...
    return {"service": "interview-prep-frontend", "domain": "frontend",
            "endpoint": "/", "method": "GET",
            "status": "ok", "data": {},
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "interview-prep-frontend", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting interview-prep-frontend port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
