"""
Security Gateway — Security Service

Template: svc_monitoring_016
Domain: security | Tier: large | Port: 8198
...
SERVICE_PORT        = config.get('port', 8198)
...
app = FastAPI(
    ...
    description="Security service | Port 8198 | Tier: large | NS: jtp",
...
async def root():
    return {
        "service": "security_gateway", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "security_gateway",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "security_gateway", "port": SERVICE_PORT,
...
async def security_status():
    ...
    return {"service": "security_gateway", "domain": "security",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "security_gateway", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting security_gateway port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
