"""E2E tests for security_gateway via gateway.
Port: 8198 | NS: jtp
...
class TestSecurityGatewayE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/security_gateway/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/security_gateway/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_security_status(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/security_gateway/security/status", timeout=30.0)
    ...
    def test_gateway_endpoint_security_scan(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/security_gateway/security/scan", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_threats(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/security_gateway/threats", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_policies_id(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/security_gateway/policies/test-id", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_alerts(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/security_gateway/alerts", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_incidents_report(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/security_gateway/incidents/report", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
