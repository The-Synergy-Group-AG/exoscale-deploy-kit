"""Unit tests for security_gateway.
Template: svc_monitoring_016 | Port: 8198 | Tier: large
...
class TestSecurityGatewayUnit:
    """Unit tests — port 8198 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "security_gateway"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8198, f"Port mismatch: expected 8198 got {d['port']}"
    ...
    def test_endpoint_get_security_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "security_gateway", \
