"""E2E tests for onboarding_service via gateway.
Port: 8236 | NS: jtp
...
class TestOnboardingServiceE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/onboarding_service/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/onboarding_service/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_users(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/onboarding_service/users", timeout=30.0)
    ...
    def test_gateway_endpoint_users(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/onboarding_service/users", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Gateway endpoint /users returned {r.status_code}"
            assert r.json() is not None
        except httpx.ConnectError:
    ...
    def test_gateway_endpoint_users_id(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/onboarding_service/users/test-id", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_users_id(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/onboarding_service/users/test-id", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Gateway endpoint /users/{id} returned {r.status_code}"
            assert r.json() is not None
        except httpx.ConnectError:
    ...
    def test_gateway_endpoint_users_id_profile(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/onboarding_service/users/test-id/profile", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
