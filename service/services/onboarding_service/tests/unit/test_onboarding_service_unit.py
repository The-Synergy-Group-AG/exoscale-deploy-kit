...
Template: svc_user_management_006 | Port: 8236 | Tier: medium
...
class TestOnboardingServiceUnit:
    """Unit tests — port 8236 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "onboarding_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8236, f"Port mismatch: expected 8236 got {d['port']}"
    ...
    def test_endpoint_get_users(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "onboarding_service", \
