"""Unit tests for advanced_ml_pipeline_service.
Template: svc_analytics_009 | Port: 8115 | Tier: large
...
class TestAdvancedMlPipelineServiceUnit:
    """Unit tests — port 8115 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "advanced_ml_pipeline_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8115, f"Port mismatch: expected 8115 got {d['port']}"
    ...
    def test_endpoint_post_ai_process(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "advanced_ml_pipeline_service", \
