"""Unit tests for deep_learning_accelerator.
Template: svc_analytics_009 | Port: 8121 | Tier: large
...
class TestDeepLearningAcceleratorUnit:
    """Unit tests — port 8121 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "deep_learning_accelerator"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8121, f"Port mismatch: expected 8121 got {d['port']}"
    ...
    def test_endpoint_post_ai_process(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "deep_learning_accelerator", \
