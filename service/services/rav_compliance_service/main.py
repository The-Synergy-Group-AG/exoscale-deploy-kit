"""
Rav Compliance Service — Compliance Service

Template: svc_audit_014
Domain: compliance | Tier: medium | Port: 8187
...
SERVICE_PORT        = config.get('port', 8187)
...
app = FastAPI(
    title="Rav Compliance Service API",
    description="Compliance service | Port 8187 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "rav_compliance_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "rav_compliance_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "rav_compliance_service", "port": SERVICE_PORT,
...
async def compliance_status():
    ...
    return {"service": "rav_compliance_service", "domain": "compliance",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "rav_compliance_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting rav_compliance_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
