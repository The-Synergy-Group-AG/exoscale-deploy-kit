"""E2E tests for rav_compliance_service via gateway.
Port: 8187 | NS: jtp
...
class TestRavComplianceServiceE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/rav_compliance_service/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/rav_compliance_service/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_compliance_status(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/rav_compliance_service/compliance/status", timeout=30.0)
    ...
    def test_gateway_endpoint_audit_logs(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/rav_compliance_service/audit/logs", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_audit_report(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/rav_compliance_service/audit/report", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_regulations(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/rav_compliance_service/regulations", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_compliance_check(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/rav_compliance_service/compliance/check", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_violations(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/rav_compliance_service/violations", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
