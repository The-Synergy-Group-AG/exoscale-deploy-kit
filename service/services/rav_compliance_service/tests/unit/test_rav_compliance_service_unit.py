"""Unit tests for rav_compliance_service.
Template: svc_audit_014 | Port: 8187 | Tier: medium
...
class TestRavComplianceServiceUnit:
    """Unit tests — port 8187 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "rav_compliance_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8187, f"Port mismatch: expected 8187 got {d['port']}"
    ...
    def test_endpoint_get_compliance_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "rav_compliance_service", \
