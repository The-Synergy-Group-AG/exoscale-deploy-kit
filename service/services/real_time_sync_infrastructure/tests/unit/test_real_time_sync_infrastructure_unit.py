"""Unit tests for real_time_sync_infrastructure.
Template: svc_workflow_012 | Port: 8188 | Tier: medium
...
class TestRealTimeSyncInfrastructureUnit:
    """Unit tests — port 8188 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "real_time_sync_infrastructure"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8188, f"Port mismatch: expected 8188 got {d['port']}"
    ...
    def test_endpoint_get_workflows(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "real_time_sync_infrastructure", \
