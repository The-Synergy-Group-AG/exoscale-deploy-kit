"""
Real Time Sync Infrastructure — Workflow Service

Template: svc_workflow_012
Domain: workflow | Tier: medium | Port: 8188
...
SERVICE_PORT        = config.get('port', 8188)
...
app = FastAPI(
    title="Real Time Sync Infrastructure API",
    description="Workflow service | Port 8188 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "real_time_sync_infrastructure", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "real_time_sync_infrastructure",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "real_time_sync_infrastructure", "port": SERVICE_PORT,
...
async def workflows():
    ...
    return {"service": "real_time_sync_infrastructure", "domain": "workflow",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "real_time_sync_infrastructure", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting real_time_sync_infrastructure port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
