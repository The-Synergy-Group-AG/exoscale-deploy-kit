"""Integration tests for performance_compliance_test_service.
Port: 8123 | Tier: medium | Namespace: jtp
...
class TestPerformanceComplianceTestServiceIntegration:
    ...
    def test_domain_endpoint_compliance_status(self, base_url):
        """Integration: GET /compliance/status responds correctly."""
        try:
            r = httpx.get(f"{base_url}/compliance/status", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /compliance/status returned {r.status_code}"
            data = r.json()
    ...
    def test_domain_endpoint_audit_logs(self, base_url):
        """Integration: GET /audit/logs responds correctly."""
        try:
            r = httpx.get(f"{base_url}/audit/logs", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /audit/logs returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
    ...
    def test_domain_endpoint_audit_report(self, base_url):
        """Integration: POST /audit/report responds correctly."""
        try:
            r = httpx.post(f"{base_url}/audit/report", json={}, timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /audit/report returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
