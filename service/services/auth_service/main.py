...
Domain: authentication | Tier: medium | Port: 8215
...
SERVICE_PORT        = config.get('port', 8215)
...
app = FastAPI(
    ...
    description="Authentication service | Port 8215 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "auth_service", "type": "backend",
...
async def metrics():
    return {
        "service": "auth_service", "port": SERVICE_PORT,
...
async def auth_status():
    ...
    return {"service": "auth_service", "domain": "authentication",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "auth_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting auth_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
