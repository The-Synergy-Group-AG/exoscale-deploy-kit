...
Template: svc_authentication_001 | Port: 8215 | Tier: medium
...
class TestAuthServiceUnit:
    """Unit tests — port 8215 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "auth_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8215, f"Port mismatch: expected 8215 got {d['port']}"
    ...
    def test_endpoint_get_auth_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "auth_service", \
