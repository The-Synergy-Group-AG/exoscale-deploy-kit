"""Unit tests for swiss_compliance_service.
Template: svc_audit_014 | Port: 8104 | Tier: medium
...
class TestSwissComplianceServiceUnit:
    """Unit tests — port 8104 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "swiss_compliance_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8104, f"Port mismatch: expected 8104 got {d['port']}"
    ...
    def test_endpoint_get_compliance_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "swiss_compliance_service", \
