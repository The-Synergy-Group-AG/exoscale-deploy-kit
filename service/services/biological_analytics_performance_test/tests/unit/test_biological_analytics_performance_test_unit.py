"""Unit tests for biological_analytics_performance_test.
Template: svc_monitoring_016 | Port: 8280 | Tier: small
...
class TestBiologicalAnalyticsPerformanceTestUnit:
    """Unit tests — port 8280 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "biological_analytics_performance_test"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8280, f"Port mismatch: expected 8280 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "biological_analytics_performance_test", \
