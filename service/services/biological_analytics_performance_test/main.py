"""
Biological Analytics Performance Test — Monitoring Service

Template: svc_monitoring_016
Domain: monitoring | Tier: small | Port: 8280
...
SERVICE_PORT        = config.get('port', 8280)
...
app = FastAPI(
    title="Biological Analytics Performance Test API",
    description="Monitoring service | Port 8280 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "biological_analytics_performance_test", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "biological_analytics_performance_test",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "biological_analytics_performance_test", "port": SERVICE_PORT,
...
async def status():
    ...
    return {"service": "biological_analytics_performance_test", "domain": "monitoring",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "biological_analytics_performance_test", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting biological_analytics_performance_test port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
