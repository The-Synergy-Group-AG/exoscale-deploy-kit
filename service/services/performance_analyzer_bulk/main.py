"""
Performance Analyzer Bulk — Monitoring Service

Template: svc_monitoring_016
Domain: monitoring | Tier: small | Port: 8296
...
SERVICE_PORT        = config.get('port', 8296)
...
app = FastAPI(
    title="Performance Analyzer Bulk API",
    description="Monitoring service | Port 8296 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "performance_analyzer_bulk", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "performance_analyzer_bulk",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "performance_analyzer_bulk", "port": SERVICE_PORT,
...
async def status():
    ...
    return {"service": "performance_analyzer_bulk", "domain": "monitoring",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "performance_analyzer_bulk", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting performance_analyzer_bulk port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
