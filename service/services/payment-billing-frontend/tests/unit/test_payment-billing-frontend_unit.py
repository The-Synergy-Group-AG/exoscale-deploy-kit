"""Unit tests for payment-billing-frontend.
Template: svc_frontend_003 | Port: 8127 | Tier: small
...
class TestPaymentBillingFrontendUnit:
    """Unit tests — port 8127 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "payment-billing-frontend"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8127, f"Port mismatch: expected 8127 got {d['port']}"
    ...
    def test_endpoint_get_root(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "payment-billing-frontend", \
