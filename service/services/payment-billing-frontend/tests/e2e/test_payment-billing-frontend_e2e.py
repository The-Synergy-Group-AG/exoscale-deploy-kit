"""E2E tests for payment-billing-frontend via gateway.
Port: 8127 | NS: jtp
...
class TestPaymentBillingFrontendE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/payment-billing-frontend/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/payment-billing-frontend/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_api_status(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/payment-billing-frontend/api/status", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_assets_manifest(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/payment-billing-frontend/assets/manifest", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
