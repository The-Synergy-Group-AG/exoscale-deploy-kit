"""Unit tests for etl_pipelines_service.
Template: svc_workflow_012 | Port: 8193 | Tier: medium
...
class TestEtlPipelinesServiceUnit:
    """Unit tests — port 8193 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "etl_pipelines_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8193, f"Port mismatch: expected 8193 got {d['port']}"
    ...
    def test_endpoint_get_workflows(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "etl_pipelines_service", \
