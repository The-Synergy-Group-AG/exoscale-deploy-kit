"""E2E tests for etl_pipelines_service via gateway.
Port: 8193 | NS: jtp
...
class TestEtlPipelinesServiceE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/etl_pipelines_service/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/etl_pipelines_service/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_workflows(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/etl_pipelines_service/workflows", timeout=30.0)
    ...
    def test_gateway_endpoint_workflows_execute(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/etl_pipelines_service/workflows/execute", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_workflows_id_status(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/etl_pipelines_service/workflows/test-id/status", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_workflows_id_cancel(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/etl_pipelines_service/workflows/test-id/cancel", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_pipelines(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/etl_pipelines_service/pipelines", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_pipelines_trigger(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/etl_pipelines_service/pipelines/trigger", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
