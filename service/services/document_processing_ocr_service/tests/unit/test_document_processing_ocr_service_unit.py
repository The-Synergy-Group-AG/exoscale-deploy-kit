"""Unit tests for document_processing_ocr_service.
Template: svc_catalog_008 | Port: 8167 | Tier: medium
...
class TestDocumentProcessingOcrServiceUnit:
    """Unit tests — port 8167 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "document_processing_ocr_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8167, f"Port mismatch: expected 8167 got {d['port']}"
    ...
    def test_endpoint_get_documents(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "document_processing_ocr_service", \
