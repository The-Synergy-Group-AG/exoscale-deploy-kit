"""Integration tests for document_processing_ocr_service.
Port: 8167 | Tier: medium | Namespace: jtp
...
class TestDocumentProcessingOcrServiceIntegration:
    ...
    def test_domain_endpoint_documents(self, base_url):
        """Integration: GET /documents responds correctly."""
        try:
            r = httpx.get(f"{base_url}/documents", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /documents returned {r.status_code}"
            data = r.json()
    ...
    def test_domain_endpoint_documents_generate(self, base_url):
        """Integration: POST /documents/generate responds correctly."""
        try:
            r = httpx.post(f"{base_url}/documents/generate", json={}, timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /documents/generate returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
    ...
    def test_domain_endpoint_documents_id(self, base_url):
        """Integration: GET /documents/{id} responds correctly."""
        try:
            r = httpx.get(f"{base_url}/documents/test-id", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /documents/{id} returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
