"""Unit tests for backup_recovery_system.
Template: svc_health_017 | Port: 8224 | Tier: small
...
class TestBackupRecoverySystemUnit:
    """Unit tests — port 8224 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "backup_recovery_system"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8224, f"Port mismatch: expected 8224 got {d['port']}"
    ...
    def test_endpoint_get_backup_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "backup_recovery_system", \
