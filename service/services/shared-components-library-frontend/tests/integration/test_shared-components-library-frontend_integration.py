"""Integration tests for shared-components-library-frontend.
Port: 8144 | Tier: small | Namespace: jtp
...
class TestSharedComponentsLibraryFrontendIntegration:
    ...
    def test_domain_endpoint_root(self, base_url):
        """Integration: GET / responds correctly."""
        try:
            r = httpx.get(f"{base_url}/", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint / returned {r.status_code}"
            data = r.json()
    ...
    def test_domain_endpoint_api_status(self, base_url):
        """Integration: GET /api/status responds correctly."""
        try:
            r = httpx.get(f"{base_url}/api/status", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /api/status returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
    ...
    def test_domain_endpoint_assets_manifest(self, base_url):
        """Integration: GET /assets/manifest responds correctly."""
        try:
            r = httpx.get(f"{base_url}/assets/manifest", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /assets/manifest returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
