"""E2E tests for shared-components-library-frontend via gateway.
Port: 8144 | NS: jtp
...
class TestSharedComponentsLibraryFrontendE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/shared-components-library-frontend/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/shared-components-library-frontend/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_api_status(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/shared-components-library-frontend/api/status", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_assets_manifest(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/shared-components-library-frontend/assets/manifest", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
