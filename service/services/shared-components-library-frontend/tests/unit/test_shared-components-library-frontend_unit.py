"""Unit tests for shared-components-library-frontend.
Template: svc_frontend_003 | Port: 8144 | Tier: small
Namespace: jtp | Domain: frontend
...
class TestSharedComponentsLibraryFrontendUnit:
    """Unit tests — port 8144 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "shared-components-library-frontend"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8144, f"Port mismatch: expected 8144 got {d['port']}"
    ...
    def test_root_returns_domain_and_template(self, client):
        ...
        assert r.status_code == 200
        d = r.json()
        assert d.get("domain") == "frontend", f"Domain mismatch: {d}"
    ...
    def test_endpoint_get_root(self, client):
        """Test GET / — status + schema validation"""
        response = client.get("/")
        assert response.status_code in (200, 201, 405, 422)
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "shared-components-library-frontend", \
                f"Response must include service name, got: {data}"
            assert data.get("domain") == "frontend", \
    ...
    def test_endpoint_get_api_status(self, client):
        """Test GET /api/status — status + schema validation"""
        ...
        if response.status_code in (200, 201):
            ...
            assert data.get("domain") == "frontend", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/api/status", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
    ...
    def test_endpoint_get_assets_manifest(self, client):
        """Test GET /assets/manifest — status + schema validation"""
        response = client.get("/assets/manifest")
        assert response.status_code in (200, 201, 405, 422)
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "shared-components-library-frontend", \
            ...
            assert data.get("domain") == "frontend", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/assets/manifest", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
