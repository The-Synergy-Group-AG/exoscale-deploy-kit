"""Unit tests for performance_optimizer.
Template: svc_monitoring_016 | Port: 8135 | Tier: small
...
class TestPerformanceOptimizerUnit:
    """Unit tests — port 8135 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "performance_optimizer"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8135, f"Port mismatch: expected 8135 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "performance_optimizer", \
