"""Unit tests for natural_language_processor.
Template: svc_authentication_001 | Port: 8300 | Tier: medium
...
class TestNaturalLanguageProcessorUnit:
    """Unit tests — port 8300 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "natural_language_processor"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8300, f"Port mismatch: expected 8300 got {d['port']}"
    ...
    def test_endpoint_get_auth_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "natural_language_processor", \
