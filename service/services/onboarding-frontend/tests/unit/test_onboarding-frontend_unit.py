"""Unit tests for onboarding-frontend.
Template: svc_frontend_003 | Port: 8093 | Tier: small
...
class TestOnboardingFrontendUnit:
    """Unit tests — port 8093 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "onboarding-frontend"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8093, f"Port mismatch: expected 8093 got {d['port']}"
    ...
    def test_endpoint_get_root(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "onboarding-frontend", \
