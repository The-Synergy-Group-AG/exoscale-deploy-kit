"""Unit tests for failover_service.
Template: svc_health_017 | Port: 8194 | Tier: small
...
class TestFailoverServiceUnit:
    """Unit tests — port 8194 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "failover_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8194, f"Port mismatch: expected 8194 got {d['port']}"
    ...
    def test_endpoint_get_backup_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "failover_service", \
