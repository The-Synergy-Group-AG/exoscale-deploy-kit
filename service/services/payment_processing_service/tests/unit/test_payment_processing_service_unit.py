"""Unit tests for payment_processing_service.
Template: svc_payment_002 | Port: 8290 | Tier: medium
...
class TestPaymentProcessingServiceUnit:
    """Unit tests — port 8290 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "payment_processing_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8290, f"Port mismatch: expected 8290 got {d['port']}"
    ...
    def test_endpoint_get_payments(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "payment_processing_service", \
