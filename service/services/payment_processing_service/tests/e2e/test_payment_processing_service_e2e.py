"""E2E tests for payment_processing_service via gateway.
Port: 8290 | NS: jtp
...
class TestPaymentProcessingServiceE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/payment_processing_service/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/payment_processing_service/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_payments(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/payment_processing_service/payments", timeout=30.0)
    ...
    def test_gateway_endpoint_payments_process(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/payment_processing_service/payments/process", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_payments_id(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/payment_processing_service/payments/test-id", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_billing_invoice(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/payment_processing_service/billing/invoice", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_subscriptions(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/payment_processing_service/subscriptions", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_credits_apply(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/payment_processing_service/credits/apply", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
