"""Unit tests for event_processing_pipelines_service.
Template: svc_workflow_012 | Port: 8091 | Tier: medium
Namespace: jtp | Domain: workflow
...
class TestEventProcessingPipelinesServiceUnit:
    """Unit tests — port 8091 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "event_processing_pipelines_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8091, f"Port mismatch: expected 8091 got {d['port']}"
    ...
    def test_root_returns_domain_and_template(self, client):
        ...
        assert r.status_code == 200
        d = r.json()
        assert d.get("domain") == "workflow", f"Domain mismatch: {d}"
    ...
    def test_endpoint_get_workflows(self, client):
        """Test GET /workflows — status + schema validation"""
        response = client.get("/workflows")
        assert response.status_code in (200, 201, 405, 422)
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "event_processing_pipelines_service", \
                f"Response must include service name, got: {data}"
            assert data.get("domain") == "workflow", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/workflows", \
    ...
    def test_endpoint_post_workflows_execute(self, client):
        """Test POST /workflows/execute — status + schema validation"""
        response = client.post("/workflows/execute")
        assert response.status_code in (200, 201, 405, 422)
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "event_processing_pipelines_service", \
            ...
            assert data.get("domain") == "workflow", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/workflows/execute", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
    ...
    def test_endpoint_get_workflows_id_status(self, client):
        """Test GET /workflows/{id}/status — status + schema validation"""
        response = client.get("/workflows/{id}/status")
        assert response.status_code in (200, 201, 405, 422)
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "event_processing_pipelines_service", \
            ...
            assert data.get("domain") == "workflow", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/workflows/{id}/status", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
    ...
    def test_endpoint_put_workflows_id_cancel(self, client):
        """Test PUT /workflows/{id}/cancel — status + schema validation"""
        response = client.put("/workflows/{id}/cancel")
        assert response.status_code in (200, 201, 405, 422)
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "event_processing_pipelines_service", \
            ...
            assert data.get("domain") == "workflow", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/workflows/{id}/cancel", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
    ...
    def test_endpoint_get_pipelines(self, client):
        """Test GET /pipelines — status + schema validation"""
        ...
        if response.status_code in (200, 201):
            ...
            assert data.get("domain") == "workflow", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/pipelines", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
    ...
    def test_endpoint_post_pipelines_trigger(self, client):
        """Test POST /pipelines/trigger — status + schema validation"""
        response = client.post("/pipelines/trigger")
        assert response.status_code in (200, 201, 405, 422)
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "event_processing_pipelines_service", \
            ...
            assert data.get("domain") == "workflow", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/pipelines/trigger", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
