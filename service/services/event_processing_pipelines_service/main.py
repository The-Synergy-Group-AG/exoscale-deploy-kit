"""
Event Processing Pipelines Service — Workflow Service

Template: svc_workflow_012
Domain: workflow | Tier: medium | Port: 8091
...
SERVICE_PORT        = config.get('port', 8091)
...
app = FastAPI(
    title="Event Processing Pipelines Service API",
    description="Workflow service | Port 8091 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "event_processing_pipelines_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "event_processing_pipelines_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "event_processing_pipelines_service", "port": SERVICE_PORT,
...
async def workflows():
    ...
    return {"service": "event_processing_pipelines_service", "domain": "workflow",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "event_processing_pipelines_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting event_processing_pipelines_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
