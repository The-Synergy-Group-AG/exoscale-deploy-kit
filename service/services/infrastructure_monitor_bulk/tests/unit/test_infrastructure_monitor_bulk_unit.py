"""Unit tests for infrastructure_monitor_bulk.
Template: svc_monitoring_016 | Port: 8200 | Tier: small
...
class TestInfrastructureMonitorBulkUnit:
    """Unit tests — port 8200 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "infrastructure_monitor_bulk"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8200, f"Port mismatch: expected 8200 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "infrastructure_monitor_bulk", \
