"""Unit tests for portal_integration_hub.
Template: svc_workflow_012 | Port: 8273 | Tier: medium
...
class TestPortalIntegrationHubUnit:
    """Unit tests — port 8273 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "portal_integration_hub"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8273, f"Port mismatch: expected 8273 got {d['port']}"
    ...
    def test_endpoint_get_workflows(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "portal_integration_hub", \
