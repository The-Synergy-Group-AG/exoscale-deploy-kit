"""
Portal Integration Hub — Workflow Service

Template: svc_workflow_012
Domain: workflow | Tier: medium | Port: 8273
...
SERVICE_PORT        = config.get('port', 8273)
...
app = FastAPI(
    title="Portal Integration Hub API",
    description="Workflow service | Port 8273 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "portal_integration_hub", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "portal_integration_hub",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "portal_integration_hub", "port": SERVICE_PORT,
...
async def workflows():
    ...
    return {"service": "portal_integration_hub", "domain": "workflow",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "portal_integration_hub", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting portal_integration_hub port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
