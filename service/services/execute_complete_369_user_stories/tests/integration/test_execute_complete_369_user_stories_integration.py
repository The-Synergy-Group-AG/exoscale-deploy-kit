"""Integration tests for execute_complete_369_user_stories.
Port: 8152 | Tier: small | Namespace: jtp
...
class TestExecuteComplete369UserStoriesIntegration:
    ...
    def test_domain_endpoint_data(self, base_url):
        """Integration: GET /data responds correctly."""
        try:
            r = httpx.get(f"{base_url}/data", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /data returned {r.status_code}"
            data = r.json()
    ...
    def test_domain_endpoint_process(self, base_url):
        """Integration: POST /process responds correctly."""
        try:
            r = httpx.post(f"{base_url}/process", json={}, timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /process returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
    ...
    def test_domain_endpoint_info(self, base_url):
        """Integration: GET /info responds correctly."""
        try:
            r = httpx.get(f"{base_url}/info", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /info returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
