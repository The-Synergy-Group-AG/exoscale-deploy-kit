"""Unit tests for execute_complete_369_user_stories.
Template: svc_catalog_008 | Port: 8152 | Tier: small
Namespace: jtp | Domain: general
...
class TestExecuteComplete369UserStoriesUnit:
    """Unit tests — port 8152 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "execute_complete_369_user_stories"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8152, f"Port mismatch: expected 8152 got {d['port']}"
    ...
    def test_root_returns_domain_and_template(self, client):
        ...
        assert r.status_code == 200
        d = r.json()
        assert d.get("domain") == "general", f"Domain mismatch: {d}"
    ...
    def test_endpoint_get_data(self, client):
        """Test GET /data — status + schema validation"""
        response = client.get("/data")
        assert response.status_code in (200, 201, 405, 422)
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "execute_complete_369_user_stories", \
                f"Response must include service name, got: {data}"
            assert data.get("domain") == "general", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/data", \
    ...
    def test_endpoint_post_process(self, client):
        """Test POST /process — status + schema validation"""
        ...
        if response.status_code in (200, 201):
            ...
            assert data.get("domain") == "general", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/process", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
    ...
    def test_endpoint_get_info(self, client):
        """Test GET /info — status + schema validation"""
    ...
    def test_endpoint_put_data_id(self, client):
        """Test PUT /data/{id} — status + schema validation"""
        response = client.put("/data/{id}")
        assert response.status_code in (200, 201, 405, 422)
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "execute_complete_369_user_stories", \
            ...
            assert data.get("domain") == "general", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/data/{id}", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
    ...
    def test_endpoint_delete_data_id(self, client):
        """Test DELETE /data/{id} — status + schema validation"""
    ...
    def test_endpoint_get_summary(self, client):
        """Test GET /summary — status + schema validation"""
        ...
        if response.status_code in (200, 201):
            ...
            assert data.get("domain") == "general", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/summary", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
