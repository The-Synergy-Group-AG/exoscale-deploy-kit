"""Unit tests for performance_optimizer_bulk.
Template: svc_monitoring_016 | Port: 8145 | Tier: small
...
class TestPerformanceOptimizerBulkUnit:
    """Unit tests — port 8145 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "performance_optimizer_bulk"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8145, f"Port mismatch: expected 8145 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "performance_optimizer_bulk", \
