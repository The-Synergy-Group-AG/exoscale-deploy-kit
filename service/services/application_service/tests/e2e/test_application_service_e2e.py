"""E2E tests for application_service via gateway.
Port: 8213 | NS: jtp
...
class TestApplicationServiceE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/application_service/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/application_service/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_data(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/application_service/data", timeout=30.0)
    ...
    def test_gateway_endpoint_process(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/application_service/process", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_info(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/application_service/info", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_data_id(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/application_service/data/test-id", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_summary(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/application_service/summary", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
