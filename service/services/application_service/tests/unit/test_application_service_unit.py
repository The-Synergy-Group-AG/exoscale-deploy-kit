...
Template: svc_catalog_008 | Port: 8213 | Tier: small
...
class TestApplicationServiceUnit:
    """Unit tests — port 8213 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "application_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8213, f"Port mismatch: expected 8213 got {d['port']}"
    ...
    def test_endpoint_get_data(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "application_service", \
