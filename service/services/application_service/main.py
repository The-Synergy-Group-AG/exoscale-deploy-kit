"""
Application Service — General Service

Template: svc_catalog_008
Domain: general | Tier: small | Port: 8213
...
SERVICE_PORT        = config.get('port', 8213)
...
app = FastAPI(
    ...
    description="General service | Port 8213 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "application_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "application_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "application_service", "port": SERVICE_PORT,
...
async def data():
    ...
    return {"service": "application_service", "domain": "general",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "application_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting application_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
