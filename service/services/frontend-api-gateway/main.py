"""
Frontend-Api-Gateway — Frontend Service

Template: svc_frontend_003
Domain: frontend | Tier: small | Port: 8103
...
SERVICE_PORT        = config.get('port', 8103)
...
app = FastAPI(
    title="Frontend-Api-Gateway API",
    description="Frontend service | Port 8103 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "frontend-api-gateway", "type": "frontend",
...
async def health():
    return {
        "status": "healthy", "service": "frontend-api-gateway",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "frontend-api-gateway", "port": SERVICE_PORT,
...
async def root():
    ...
    return {"service": "frontend-api-gateway", "domain": "frontend",
            "endpoint": "/", "method": "GET",
            "status": "ok", "data": {},
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "frontend-api-gateway", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting frontend-api-gateway port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
