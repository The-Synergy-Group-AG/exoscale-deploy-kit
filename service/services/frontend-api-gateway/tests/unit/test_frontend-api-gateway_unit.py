"""Unit tests for frontend-api-gateway.
Template: svc_frontend_003 | Port: 8103 | Tier: small
...
class TestFrontendApiGatewayUnit:
    """Unit tests — port 8103 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "frontend-api-gateway"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8103, f"Port mismatch: expected 8103 got {d['port']}"
    ...
    def test_endpoint_get_root(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "frontend-api-gateway", \
