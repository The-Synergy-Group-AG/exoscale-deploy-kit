"""Unit tests for payment_processor_service.
Template: svc_authentication_001 | Port: 8172 | Tier: medium
...
class TestPaymentProcessorServiceUnit:
    """Unit tests — port 8172 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "payment_processor_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8172, f"Port mismatch: expected 8172 got {d['port']}"
    ...
    def test_endpoint_get_auth_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "payment_processor_service", \
