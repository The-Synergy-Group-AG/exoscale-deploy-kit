"""
Enterprise Sso Integration Service — Authentication Service

Template: svc_authentication_001
Domain: authentication | Tier: medium | Port: 8277
...
SERVICE_PORT        = config.get('port', 8277)
...
app = FastAPI(
    title="Enterprise Sso Integration Service API",
    description="Authentication service | Port 8277 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "enterprise_sso_integration_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "enterprise_sso_integration_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "enterprise_sso_integration_service", "port": SERVICE_PORT,
...
async def auth_status():
    ...
    return {"service": "enterprise_sso_integration_service", "domain": "authentication",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "enterprise_sso_integration_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting enterprise_sso_integration_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
