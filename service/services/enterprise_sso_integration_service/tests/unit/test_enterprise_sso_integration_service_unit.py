"""Unit tests for enterprise_sso_integration_service.
Template: svc_authentication_001 | Port: 8277 | Tier: medium
...
class TestEnterpriseSsoIntegrationServiceUnit:
    """Unit tests — port 8277 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "enterprise_sso_integration_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8277, f"Port mismatch: expected 8277 got {d['port']}"
    ...
    def test_endpoint_get_auth_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "enterprise_sso_integration_service", \
