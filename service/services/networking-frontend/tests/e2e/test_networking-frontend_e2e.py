"""E2E tests for networking-frontend via gateway.
Port: 8133 | NS: jtp
...
class TestNetworkingFrontendE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/networking-frontend/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/networking-frontend/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_api_status(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/networking-frontend/api/status", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_assets_manifest(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/networking-frontend/assets/manifest", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
