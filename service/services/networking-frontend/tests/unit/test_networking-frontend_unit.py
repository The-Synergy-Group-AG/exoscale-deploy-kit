"""Unit tests for networking-frontend.
Template: svc_frontend_003 | Port: 8133 | Tier: small
...
class TestNetworkingFrontendUnit:
    """Unit tests — port 8133 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "networking-frontend"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8133, f"Port mismatch: expected 8133 got {d['port']}"
    ...
    def test_endpoint_get_root(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "networking-frontend", \
