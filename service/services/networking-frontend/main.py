"""
Networking-Frontend — Frontend Service

Template: svc_frontend_003
Domain: frontend | Tier: small | Port: 8133
...
SERVICE_PORT        = config.get('port', 8133)
...
app = FastAPI(
    title="Networking-Frontend API",
    description="Frontend service | Port 8133 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "networking-frontend", "type": "frontend",
...
async def health():
    return {
        "status": "healthy", "service": "networking-frontend",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "networking-frontend", "port": SERVICE_PORT,
...
async def root():
    ...
    return {"service": "networking-frontend", "domain": "frontend",
            "endpoint": "/", "method": "GET",
            "status": "ok", "data": {},
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "networking-frontend", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting networking-frontend port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
