"""Unit tests for application_pipeline_service.
Template: svc_workflow_012 | Port: 8298 | Tier: medium
...
class TestApplicationPipelineServiceUnit:
    """Unit tests — port 8298 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "application_pipeline_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8298, f"Port mismatch: expected 8298 got {d['port']}"
    ...
    def test_endpoint_get_workflows(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "application_pipeline_service", \
