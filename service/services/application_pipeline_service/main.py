"""
Application Pipeline Service — Workflow Service

Template: svc_workflow_012
Domain: workflow | Tier: medium | Port: 8298
...
SERVICE_PORT        = config.get('port', 8298)
...
app = FastAPI(
    title="Application Pipeline Service API",
    description="Workflow service | Port 8298 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "application_pipeline_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "application_pipeline_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "application_pipeline_service", "port": SERVICE_PORT,
...
async def workflows():
    ...
    return {"service": "application_pipeline_service", "domain": "workflow",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "application_pipeline_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting application_pipeline_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
