"""Unit tests for regression_detection_service.
Template: svc_monitoring_016 | Port: 8124 | Tier: small
...
class TestRegressionDetectionServiceUnit:
    """Unit tests — port 8124 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "regression_detection_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8124, f"Port mismatch: expected 8124 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "regression_detection_service", \
