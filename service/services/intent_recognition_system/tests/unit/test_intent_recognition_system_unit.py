"""Unit tests for intent_recognition_system.
Template: svc_analytics_009 | Port: 8140 | Tier: large
...
class TestIntentRecognitionSystemUnit:
    """Unit tests — port 8140 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "intent_recognition_system"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8140, f"Port mismatch: expected 8140 got {d['port']}"
    ...
    def test_endpoint_post_ai_process(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "intent_recognition_system", \
