"""Unit tests for api_gateway_service.
Template: svc_monitoring_016 | Port: 8015 | Tier: large
...
class TestApiGatewayServiceUnit:
    """Unit tests — port 8015 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "api_gateway_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8015, f"Port mismatch: expected 8015 got {d['port']}"
    ...
    def test_endpoint_get_security_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "api_gateway_service", \
