"""
Api Gateway Service — Security Service

Template: svc_monitoring_016
Domain: security | Tier: large | Port: 8015
...
SERVICE_PORT        = config.get('port', 8015)
...
app = FastAPI(
    ...
    description="Security service | Port 8015 | Tier: large | NS: jtp",
...
async def root():
    return {
        "service": "api_gateway_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "api_gateway_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "api_gateway_service", "port": SERVICE_PORT,
...
async def security_status():
    ...
    return {"service": "api_gateway_service", "domain": "security",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "api_gateway_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting api_gateway_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
