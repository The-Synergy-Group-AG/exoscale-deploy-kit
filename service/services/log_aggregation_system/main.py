"""
Log Aggregation System — Monitoring Service

Template: svc_monitoring_016
Domain: monitoring | Tier: small | Port: 8185
...
SERVICE_PORT        = config.get('port', 8185)
...
app = FastAPI(
    title="Log Aggregation System API",
    description="Monitoring service | Port 8185 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "log_aggregation_system", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "log_aggregation_system",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "log_aggregation_system", "port": SERVICE_PORT,
...
async def status():
    ...
    return {"service": "log_aggregation_system", "domain": "monitoring",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "log_aggregation_system", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting log_aggregation_system port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
