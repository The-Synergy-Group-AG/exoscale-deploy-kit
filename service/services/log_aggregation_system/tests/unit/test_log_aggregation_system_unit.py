"""Unit tests for log_aggregation_system.
Template: svc_monitoring_016 | Port: 8185 | Tier: small
...
class TestLogAggregationSystemUnit:
    """Unit tests — port 8185 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "log_aggregation_system"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8185, f"Port mismatch: expected 8185 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "log_aggregation_system", \
