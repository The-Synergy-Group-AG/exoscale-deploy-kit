"""Unit tests for cv_builder_service.
Template: svc_catalog_008 | Port: 8128 | Tier: medium
...
class TestCvBuilderServiceUnit:
    """Unit tests — port 8128 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "cv_builder_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8128, f"Port mismatch: expected 8128 got {d['port']}"
    ...
    def test_endpoint_get_jobs(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "cv_builder_service", \
