"""
Cv Builder Service — Career Service

Template: svc_catalog_008
Domain: career | Tier: medium | Port: 8128
...
SERVICE_PORT        = config.get('port', 8128)
...
app = FastAPI(
    title="Cv Builder Service API",
    description="Career service | Port 8128 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "cv_builder_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "cv_builder_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "cv_builder_service", "port": SERVICE_PORT,
...
async def jobs():
    ...
    return {"service": "cv_builder_service", "domain": "career",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "cv_builder_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting cv_builder_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
