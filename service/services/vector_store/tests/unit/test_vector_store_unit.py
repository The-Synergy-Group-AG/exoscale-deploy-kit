...
Template: svc_analytics_009 | Port: 8113 | Tier: large
...
class TestVectorStoreUnit:
    """Unit tests — port 8113 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "vector_store"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8113, f"Port mismatch: expected 8113 got {d['port']}"
    ...
    def test_endpoint_post_ai_process(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "vector_store", \
