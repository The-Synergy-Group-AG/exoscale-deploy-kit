"""E2E tests for vector_store via gateway.
Port: 8113 | NS: jtp
...
class TestVectorStoreE2E:
    ...
    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/vector_store/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/vector_store/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_ai_process(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/vector_store/ai/process", json={}, timeout=30.0)
    ...
    def test_gateway_endpoint_models(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/vector_store/models", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_models_predict(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/vector_store/models/predict", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_training_status(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/vector_store/training/status", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_pipeline_run(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/vector_store/pipeline/run", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_predictions_id(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/vector_store/predictions/test-id", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
