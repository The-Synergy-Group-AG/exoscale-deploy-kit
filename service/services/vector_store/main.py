...
Domain: ai | Tier: large | Port: 8113
...
SERVICE_PORT        = config.get('port', 8113)
...
app = FastAPI(
    ...
    description="Ai service | Port 8113 | Tier: large | NS: jtp",
...
async def root():
    return {
        "service": "vector_store", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "vector_store",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "vector_store", "port": SERVICE_PORT,
...
async def ai_process(request: dict = None):
    ...
    return {"service": "vector_store", "domain": "ai",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "vector_store", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting vector_store port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
