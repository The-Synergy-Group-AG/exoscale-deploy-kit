"""Unit tests for analytics_dashboards_service.
Template: svc_analytics_009 | Port: 8266 | Tier: medium
...
class TestAnalyticsDashboardsServiceUnit:
    """Unit tests — port 8266 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "analytics_dashboards_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8266, f"Port mismatch: expected 8266 got {d['port']}"
    ...
    def test_endpoint_get_analytics_dashboard(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "analytics_dashboards_service", \
