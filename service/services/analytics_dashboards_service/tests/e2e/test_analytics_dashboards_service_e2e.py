"""E2E tests for analytics_dashboards_service via gateway.
Port: 8266 | NS: jtp
...
class TestAnalyticsDashboardsServiceE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/analytics_dashboards_service/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/analytics_dashboards_service/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_analytics_dashboard(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/analytics_dashboards_service/analytics/dashboard", timeout=30.0)
    ...
    def test_gateway_endpoint_analytics_metrics(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/analytics_dashboards_service/analytics/metrics", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_analytics_track(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/analytics_dashboards_service/analytics/track", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_reports(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/analytics_dashboards_service/reports", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_data_summary(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/analytics_dashboards_service/data/summary", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_analytics_export(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/analytics_dashboards_service/analytics/export", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
