"""
Threat Intelligence — Security Service

Template: svc_monitoring_016
Domain: security | Tier: large | Port: 8148
...
SERVICE_PORT        = config.get('port', 8148)
...
app = FastAPI(
    ...
    description="Security service | Port 8148 | Tier: large | NS: jtp",
...
async def root():
    return {
        "service": "threat_intelligence", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "threat_intelligence",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "threat_intelligence", "port": SERVICE_PORT,
...
async def security_status():
    ...
    return {"service": "threat_intelligence", "domain": "security",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "threat_intelligence", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting threat_intelligence port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
