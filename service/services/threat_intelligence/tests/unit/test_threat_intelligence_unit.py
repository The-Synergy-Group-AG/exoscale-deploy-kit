"""Unit tests for threat_intelligence.
Template: svc_monitoring_016 | Port: 8148 | Tier: large
...
class TestThreatIntelligenceUnit:
    """Unit tests — port 8148 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "threat_intelligence"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8148, f"Port mismatch: expected 8148 got {d['port']}"
    ...
    def test_endpoint_get_security_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "threat_intelligence", \
