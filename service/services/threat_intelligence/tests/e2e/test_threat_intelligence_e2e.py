"""E2E tests for threat_intelligence via gateway.
Port: 8148 | NS: jtp
...
class TestThreatIntelligenceE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/threat_intelligence/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/threat_intelligence/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_security_status(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/threat_intelligence/security/status", timeout=30.0)
    ...
    def test_gateway_endpoint_security_scan(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/threat_intelligence/security/scan", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_threats(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/threat_intelligence/threats", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_policies_id(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/threat_intelligence/policies/test-id", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_alerts(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/threat_intelligence/alerts", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_incidents_report(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/threat_intelligence/incidents/report", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
