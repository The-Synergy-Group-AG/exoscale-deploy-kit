"""Unit tests for network_security_gateway.
Template: svc_monitoring_016 | Port: 8245 | Tier: large
...
class TestNetworkSecurityGatewayUnit:
    """Unit tests — port 8245 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "network_security_gateway"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8245, f"Port mismatch: expected 8245 got {d['port']}"
    ...
    def test_endpoint_get_security_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "network_security_gateway", \
