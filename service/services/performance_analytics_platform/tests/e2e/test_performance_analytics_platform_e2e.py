"""E2E tests for performance_analytics_platform via gateway.
Port: 8138 | NS: jtp
...
class TestPerformanceAnalyticsPlatformE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/performance_analytics_platform/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/performance_analytics_platform/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_status(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/performance_analytics_platform/status", timeout=30.0)
    ...
    def test_gateway_endpoint_metrics_live(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/performance_analytics_platform/metrics/live", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_alerts_create(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/performance_analytics_platform/alerts/create", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_logs(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/performance_analytics_platform/logs", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_performance(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/performance_analytics_platform/performance", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_alerts_id(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/performance_analytics_platform/alerts/test-id", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
