"""Unit tests for performance_analytics_platform.
Template: svc_monitoring_016 | Port: 8138 | Tier: small
...
class TestPerformanceAnalyticsPlatformUnit:
    """Unit tests — port 8138 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "performance_analytics_platform"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8138, f"Port mismatch: expected 8138 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "performance_analytics_platform", \
