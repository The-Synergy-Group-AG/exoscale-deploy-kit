"""Unit tests for immune_system.
Template: svc_health_017 | Port: 8134 | Tier: large
...
class TestImmuneSystemUnit:
    """Unit tests — port 8134 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "immune_system"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8134, f"Port mismatch: expected 8134 got {d['port']}"
    ...
    def test_endpoint_get_biological_harmony(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "immune_system", \
