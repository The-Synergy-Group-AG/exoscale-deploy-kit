"""Unit tests for business_rules_service.
Template: svc_workflow_012 | Port: 8161 | Tier: medium
...
class TestBusinessRulesServiceUnit:
    """Unit tests — port 8161 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "business_rules_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8161, f"Port mismatch: expected 8161 got {d['port']}"
    ...
    def test_endpoint_get_workflows(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "business_rules_service", \
