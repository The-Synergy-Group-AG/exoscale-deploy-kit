"""Unit tests for ai_biological_synthesis_engine.
Template: svc_analytics_009 | Port: 8209 | Tier: large
...
class TestAiBiologicalSynthesisEngineUnit:
    """Unit tests — port 8209 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "ai_biological_synthesis_engine"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8209, f"Port mismatch: expected 8209 got {d['port']}"
    ...
    def test_endpoint_post_ai_process(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "ai_biological_synthesis_engine", \
