"""Unit tests for monitoring_system_bulk.
Template: svc_monitoring_016 | Port: 8159 | Tier: small
...
class TestMonitoringSystemBulkUnit:
    """Unit tests — port 8159 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "monitoring_system_bulk"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8159, f"Port mismatch: expected 8159 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "monitoring_system_bulk", \
