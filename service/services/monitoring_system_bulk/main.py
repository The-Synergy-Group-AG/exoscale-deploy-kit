"""
Monitoring System Bulk — Monitoring Service

Template: svc_monitoring_016
Domain: monitoring | Tier: small | Port: 8159
...
SERVICE_PORT        = config.get('port', 8159)
...
app = FastAPI(
    title="Monitoring System Bulk API",
    description="Monitoring service | Port 8159 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "monitoring_system_bulk", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "monitoring_system_bulk",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "monitoring_system_bulk", "port": SERVICE_PORT,
...
async def status():
    ...
    return {"service": "monitoring_system_bulk", "domain": "monitoring",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "monitoring_system_bulk", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting monitoring_system_bulk port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
