"""
Authentication Middleware Api — Authentication Service

Template: svc_authentication_001
Domain: authentication | Tier: medium | Port: 8288
...
SERVICE_PORT        = config.get('port', 8288)
...
app = FastAPI(
    title="Authentication Middleware Api API",
    description="Authentication service | Port 8288 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "authentication_middleware_api", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "authentication_middleware_api",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "authentication_middleware_api", "port": SERVICE_PORT,
...
async def auth_status():
    ...
    return {"service": "authentication_middleware_api", "domain": "authentication",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "authentication_middleware_api", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting authentication_middleware_api port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
