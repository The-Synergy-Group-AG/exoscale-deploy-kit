"""
Infrastructure Automation — Workflow Service

Template: svc_workflow_012
Domain: workflow | Tier: medium | Port: 8270
...
SERVICE_PORT        = config.get('port', 8270)
...
app = FastAPI(
    ...
    description="Workflow service | Port 8270 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "infrastructure_automation", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "infrastructure_automation",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "infrastructure_automation", "port": SERVICE_PORT,
...
async def workflows():
    ...
    return {"service": "infrastructure_automation", "domain": "workflow",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "infrastructure_automation", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting infrastructure_automation port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
