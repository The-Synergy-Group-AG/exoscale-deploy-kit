"""Unit tests for infrastructure_automation.
Template: svc_workflow_012 | Port: 8270 | Tier: medium
...
class TestInfrastructureAutomationUnit:
    """Unit tests — port 8270 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "infrastructure_automation"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8270, f"Port mismatch: expected 8270 got {d['port']}"
    ...
    def test_endpoint_get_workflows(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "infrastructure_automation", \
