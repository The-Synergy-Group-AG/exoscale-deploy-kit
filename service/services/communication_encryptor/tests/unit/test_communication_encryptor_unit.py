"""Unit tests for communication_encryptor.
Template: svc_monitoring_016 | Port: 8286 | Tier: large
...
class TestCommunicationEncryptorUnit:
    """Unit tests — port 8286 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "communication_encryptor"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8286, f"Port mismatch: expected 8286 got {d['port']}"
    ...
    def test_endpoint_get_security_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "communication_encryptor", \
