"""Unit tests for test_framework_unittest_integration.
Template: svc_monitoring_016 | Port: 8265 | Tier: small
...
class TestTestFrameworkUnittestIntegrationUnit:
    """Unit tests — port 8265 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "test_framework_unittest_integration"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8265, f"Port mismatch: expected 8265 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "test_framework_unittest_integration", \
