"""
Test Framework Unittest Integration — Monitoring Service

Template: svc_monitoring_016
Domain: monitoring | Tier: small | Port: 8265
...
SERVICE_PORT        = config.get('port', 8265)
...
app = FastAPI(
    title="Test Framework Unittest Integration API",
    description="Monitoring service | Port 8265 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "test_framework_unittest_integration", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "test_framework_unittest_integration",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "test_framework_unittest_integration", "port": SERVICE_PORT,
...
async def status():
    ...
    return {"service": "test_framework_unittest_integration", "domain": "monitoring",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "test_framework_unittest_integration", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting test_framework_unittest_integration port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
