"""Unit tests for decision_support_service.
Template: svc_analytics_009 | Port: 8110 | Tier: large
...
class TestDecisionSupportServiceUnit:
    """Unit tests — port 8110 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "decision_support_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8110, f"Port mismatch: expected 8110 got {d['port']}"
    ...
    def test_endpoint_post_ai_process(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "decision_support_service", \
