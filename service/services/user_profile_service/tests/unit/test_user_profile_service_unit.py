...
Template: svc_user_management_006 | Port: 8142 | Tier: medium
...
class TestUserProfileServiceUnit:
    """Unit tests — port 8142 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "user_profile_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8142, f"Port mismatch: expected 8142 got {d['port']}"
    ...
    def test_endpoint_get_users(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "user_profile_service", \
