"""
User Profile Service — User_Management Service

Template: svc_user_management_006
Domain: user_management | Tier: medium | Port: 8142
...
SERVICE_PORT        = config.get('port', 8142)
...
app = FastAPI(
    ...
    description="User_Management service | Port 8142 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "user_profile_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "user_profile_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "user_profile_service", "port": SERVICE_PORT,
...
async def users():
    ...
    return {"service": "user_profile_service", "domain": "user_management",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "user_profile_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting user_profile_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
