"""Unit tests for docker_orchestration.
Template: svc_workflow_012 | Port: 8174 | Tier: medium
...
class TestDockerOrchestrationUnit:
    """Unit tests — port 8174 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "docker_orchestration"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8174, f"Port mismatch: expected 8174 got {d['port']}"
    ...
    def test_endpoint_get_workflows(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "docker_orchestration", \
