"""
Tenant Management Service — User_Management Service

Template: svc_user_management_006
Domain: user_management | Tier: medium | Port: 8218
...
SERVICE_PORT        = config.get('port', 8218)
...
app = FastAPI(
    ...
    description="User_Management service | Port 8218 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "tenant_management_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "tenant_management_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "tenant_management_service", "port": SERVICE_PORT,
...
async def users():
    ...
    return {"service": "tenant_management_service", "domain": "user_management",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "tenant_management_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting tenant_management_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
