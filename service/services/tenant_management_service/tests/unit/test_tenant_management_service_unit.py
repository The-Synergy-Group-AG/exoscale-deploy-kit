"""Unit tests for tenant_management_service.
Template: svc_user_management_006 | Port: 8218 | Tier: medium
...
class TestTenantManagementServiceUnit:
    """Unit tests — port 8218 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "tenant_management_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8218, f"Port mismatch: expected 8218 got {d['port']}"
    ...
    def test_endpoint_get_users(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "tenant_management_service", \
