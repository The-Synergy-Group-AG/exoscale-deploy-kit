"""Unit tests for swiss_market_service.
Template: svc_audit_014 | Port: 8183 | Tier: medium
...
class TestSwissMarketServiceUnit:
    """Unit tests — port 8183 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "swiss_market_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8183, f"Port mismatch: expected 8183 got {d['port']}"
    ...
    def test_endpoint_get_compliance_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "swiss_market_service", \
