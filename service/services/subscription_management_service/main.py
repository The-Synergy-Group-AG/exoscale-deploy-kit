"""
Subscription Management Service — Payment Service

Template: svc_payment_002
Domain: payment | Tier: medium | Port: 8293
...
SERVICE_PORT        = config.get('port', 8293)
...
app = FastAPI(
    ...
    description="Payment service | Port 8293 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "subscription_management_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "subscription_management_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "subscription_management_service", "port": SERVICE_PORT,
...
async def payments():
    ...
    return {"service": "subscription_management_service", "domain": "payment",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "subscription_management_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting subscription_management_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
