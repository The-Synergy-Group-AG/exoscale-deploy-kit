"""Unit tests for subscription_management_service.
Template: svc_payment_002 | Port: 8293 | Tier: medium
...
class TestSubscriptionManagementServiceUnit:
    """Unit tests — port 8293 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "subscription_management_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8293, f"Port mismatch: expected 8293 got {d['port']}"
    ...
    def test_endpoint_get_payments(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "subscription_management_service", \
