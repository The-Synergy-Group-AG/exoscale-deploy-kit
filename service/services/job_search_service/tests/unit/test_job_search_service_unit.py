"""Unit tests for job_search_service.
Template: svc_catalog_008 | Port: 8285 | Tier: medium
...
class TestJobSearchServiceUnit:
    """Unit tests — port 8285 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "job_search_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8285, f"Port mismatch: expected 8285 got {d['port']}"
    ...
    def test_endpoint_get_jobs(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "job_search_service", \
