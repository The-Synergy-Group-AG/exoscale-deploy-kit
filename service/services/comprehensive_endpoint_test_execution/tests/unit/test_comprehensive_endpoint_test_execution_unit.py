"""Unit tests for comprehensive_endpoint_test_execution.
Template: svc_monitoring_016 | Port: 8122 | Tier: small
...
class TestComprehensiveEndpointTestExecutionUnit:
    """Unit tests — port 8122 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "comprehensive_endpoint_test_execution"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8122, f"Port mismatch: expected 8122 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "comprehensive_endpoint_test_execution", \
