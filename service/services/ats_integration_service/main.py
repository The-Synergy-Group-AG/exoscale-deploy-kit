"""
Ats Integration Service — Workflow Service

Template: svc_workflow_012
Domain: workflow | Tier: medium | Port: 8112
...
SERVICE_PORT        = config.get('port', 8112)
...
app = FastAPI(
    title="Ats Integration Service API",
    description="Workflow service | Port 8112 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "ats_integration_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "ats_integration_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "ats_integration_service", "port": SERVICE_PORT,
...
async def workflows():
    ...
    return {"service": "ats_integration_service", "domain": "workflow",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "ats_integration_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting ats_integration_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
