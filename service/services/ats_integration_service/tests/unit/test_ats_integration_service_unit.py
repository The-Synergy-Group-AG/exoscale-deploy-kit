"""Unit tests for ats_integration_service.
Template: svc_workflow_012 | Port: 8112 | Tier: medium
...
class TestAtsIntegrationServiceUnit:
    """Unit tests — port 8112 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "ats_integration_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8112, f"Port mismatch: expected 8112 got {d['port']}"
    ...
    def test_endpoint_get_workflows(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "ats_integration_service", \
