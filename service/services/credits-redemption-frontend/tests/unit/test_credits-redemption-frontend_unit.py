"""Unit tests for credits-redemption-frontend.
Template: svc_frontend_003 | Port: 8108 | Tier: small
...
class TestCreditsRedemptionFrontendUnit:
    """Unit tests — port 8108 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "credits-redemption-frontend"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8108, f"Port mismatch: expected 8108 got {d['port']}"
    ...
    def test_endpoint_get_root(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "credits-redemption-frontend", \
