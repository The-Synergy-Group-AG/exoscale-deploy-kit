"""Unit tests for ui_component_library_service.
Template: svc_catalog_008 | Port: 8181 | Tier: small
...
class TestUiComponentLibraryServiceUnit:
    """Unit tests — port 8181 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "ui_component_library_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8181, f"Port mismatch: expected 8181 got {d['port']}"
    ...
    def test_endpoint_get_data(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "ui_component_library_service", \
