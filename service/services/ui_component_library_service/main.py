"""
Ui Component Library Service — General Service

Template: svc_catalog_008
Domain: general | Tier: small | Port: 8181
...
SERVICE_PORT        = config.get('port', 8181)
...
app = FastAPI(
    title="Ui Component Library Service API",
    description="General service | Port 8181 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "ui_component_library_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "ui_component_library_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "ui_component_library_service", "port": SERVICE_PORT,
...
async def data():
    ...
    return {"service": "ui_component_library_service", "domain": "general",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "ui_component_library_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting ui_component_library_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
