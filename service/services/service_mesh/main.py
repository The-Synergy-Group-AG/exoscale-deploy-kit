"""
Service Mesh — Workflow Service

Template: svc_workflow_012
Domain: workflow | Tier: medium | Port: 8243
...
SERVICE_PORT        = config.get('port', 8243)
...
app = FastAPI(
    ...
    description="Workflow service | Port 8243 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "service_mesh", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "service_mesh",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "service_mesh", "port": SERVICE_PORT,
...
async def workflows():
    ...
    return {"service": "service_mesh", "domain": "workflow",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "service_mesh", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting service_mesh port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
