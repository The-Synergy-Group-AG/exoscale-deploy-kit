"""Unit tests for service_mesh.
Template: svc_workflow_012 | Port: 8243 | Tier: medium
...
class TestServiceMeshUnit:
    """Unit tests — port 8243 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "service_mesh"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8243, f"Port mismatch: expected 8243 got {d['port']}"
    ...
    def test_endpoint_get_workflows(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "service_mesh", \
