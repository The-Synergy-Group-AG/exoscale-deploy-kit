"""
Cv Documents Infrastructure — Career Service

Template: svc_catalog_008
Domain: career | Tier: medium | Port: 8283
...
SERVICE_PORT        = config.get('port', 8283)
...
app = FastAPI(
    title="Cv Documents Infrastructure API",
    description="Career service | Port 8283 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "cv_documents_infrastructure", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "cv_documents_infrastructure",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "cv_documents_infrastructure", "port": SERVICE_PORT,
...
async def jobs():
    ...
    return {"service": "cv_documents_infrastructure", "domain": "career",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "cv_documents_infrastructure", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting cv_documents_infrastructure port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
