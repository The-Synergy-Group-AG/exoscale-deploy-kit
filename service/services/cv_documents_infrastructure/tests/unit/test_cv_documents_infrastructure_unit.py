"""Unit tests for cv_documents_infrastructure.
Template: svc_catalog_008 | Port: 8283 | Tier: medium
...
class TestCvDocumentsInfrastructureUnit:
    """Unit tests — port 8283 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "cv_documents_infrastructure"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8283, f"Port mismatch: expected 8283 got {d['port']}"
    ...
    def test_endpoint_get_jobs(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "cv_documents_infrastructure", \
