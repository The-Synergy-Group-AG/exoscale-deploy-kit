"""Integration tests for interview_intelligence_infrastructure.
Port: 8242 | Tier: medium | Namespace: jtp
...
class TestInterviewIntelligenceInfrastructureIntegration:
    ...
    def test_domain_endpoint_jobs(self, base_url):
        """Integration: GET /jobs responds correctly."""
        try:
            r = httpx.get(f"{base_url}/jobs", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /jobs returned {r.status_code}"
            data = r.json()
    ...
    def test_domain_endpoint_applications(self, base_url):
        """Integration: POST /applications responds correctly."""
        try:
            r = httpx.post(f"{base_url}/applications", json={}, timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /applications returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
    ...
    def test_domain_endpoint_interviews(self, base_url):
        """Integration: GET /interviews responds correctly."""
        try:
            r = httpx.get(f"{base_url}/interviews", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /interviews returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
