"""
Interview Intelligence Infrastructure — Career Service

Template: svc_catalog_008
Domain: career | Tier: medium | Port: 8242
...
SERVICE_PORT        = config.get('port', 8242)
...
app = FastAPI(
    title="Interview Intelligence Infrastructure API",
    description="Career service | Port 8242 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "interview_intelligence_infrastructure", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "interview_intelligence_infrastructure",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "interview_intelligence_infrastructure", "port": SERVICE_PORT,
...
async def jobs():
    ...
    return {"service": "interview_intelligence_infrastructure", "domain": "career",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "interview_intelligence_infrastructure", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting interview_intelligence_infrastructure port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
