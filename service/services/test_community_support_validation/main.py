"""
Test Community Support Validation — Monitoring Service

Template: svc_monitoring_016
Domain: monitoring | Tier: small | Port: 8295
...
SERVICE_PORT        = config.get('port', 8295)
...
app = FastAPI(
    title="Test Community Support Validation API",
    description="Monitoring service | Port 8295 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "test_community_support_validation", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "test_community_support_validation",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "test_community_support_validation", "port": SERVICE_PORT,
...
async def status():
    ...
    return {"service": "test_community_support_validation", "domain": "monitoring",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "test_community_support_validation", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting test_community_support_validation port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
