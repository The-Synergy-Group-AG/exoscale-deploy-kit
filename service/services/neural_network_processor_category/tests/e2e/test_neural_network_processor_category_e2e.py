"""E2E tests for neural_network_processor_category via gateway.
Port: 8291 | NS: jtp
...
class TestNeuralNetworkProcessorCategoryE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/neural_network_processor_category/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/neural_network_processor_category/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_auth_status(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/neural_network_processor_category/auth/status", timeout=30.0)
    ...
    def test_gateway_endpoint_auth_login(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/neural_network_processor_category/auth/login", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_auth_logout(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/neural_network_processor_category/auth/logout", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_auth_refresh(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/neural_network_processor_category/auth/refresh", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_users_user_id(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/neural_network_processor_category/users/test-id", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_auth_verify(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/neural_network_processor_category/auth/verify", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
