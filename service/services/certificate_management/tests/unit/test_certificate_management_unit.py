"""Unit tests for certificate_management.
Template: svc_monitoring_016 | Port: 8100 | Tier: large
...
class TestCertificateManagementUnit:
    """Unit tests — port 8100 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "certificate_management"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8100, f"Port mismatch: expected 8100 got {d['port']}"
    ...
    def test_endpoint_get_security_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "certificate_management", \
