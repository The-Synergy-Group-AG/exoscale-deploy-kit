"""
Certificate Management — Security Service

Template: svc_monitoring_016
Domain: security | Tier: large | Port: 8100
...
SERVICE_PORT        = config.get('port', 8100)
...
app = FastAPI(
    ...
    description="Security service | Port 8100 | Tier: large | NS: jtp",
...
async def root():
    return {
        "service": "certificate_management", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "certificate_management",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "certificate_management", "port": SERVICE_PORT,
...
async def security_status():
    ...
    return {"service": "certificate_management", "domain": "security",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "certificate_management", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting certificate_management port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
