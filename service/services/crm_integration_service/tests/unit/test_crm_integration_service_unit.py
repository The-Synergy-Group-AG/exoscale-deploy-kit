"""Unit tests for crm_integration_service.
Template: svc_workflow_012 | Port: 8271 | Tier: medium
...
class TestCrmIntegrationServiceUnit:
    """Unit tests — port 8271 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "crm_integration_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8271, f"Port mismatch: expected 8271 got {d['port']}"
    ...
    def test_endpoint_get_workflows(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "crm_integration_service", \
