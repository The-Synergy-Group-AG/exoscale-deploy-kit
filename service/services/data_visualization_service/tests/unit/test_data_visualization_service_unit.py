"""Unit tests for data_visualization_service.
Template: svc_analytics_009 | Port: 8275 | Tier: medium
...
class TestDataVisualizationServiceUnit:
    """Unit tests — port 8275 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "data_visualization_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8275, f"Port mismatch: expected 8275 got {d['port']}"
    ...
    def test_endpoint_get_analytics_dashboard(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "data_visualization_service", \
