"""
Data Visualization Service — Analytics Service

Template: svc_analytics_009
Domain: analytics | Tier: medium | Port: 8275
...
SERVICE_PORT        = config.get('port', 8275)
...
app = FastAPI(
    title="Data Visualization Service API",
    description="Analytics service | Port 8275 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "data_visualization_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "data_visualization_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "data_visualization_service", "port": SERVICE_PORT,
...
async def analytics_dashboard():
    ...
    return {"service": "data_visualization_service", "domain": "analytics",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "data_visualization_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting data_visualization_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
