"""Unit tests for service_health_monitor.
Template: svc_monitoring_016 | Port: 8250 | Tier: small
...
class TestServiceHealthMonitorUnit:
    """Unit tests — port 8250 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "service_health_monitor"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8250, f"Port mismatch: expected 8250 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "service_health_monitor", \
