"""
Service Health Monitor — Monitoring Service

Template: svc_monitoring_016
Domain: monitoring | Tier: small | Port: 8250
...
SERVICE_PORT        = config.get('port', 8250)
...
app = FastAPI(
    title="Service Health Monitor API",
    description="Monitoring service | Port 8250 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "service_health_monitor", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "service_health_monitor",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "service_health_monitor", "port": SERVICE_PORT,
...
async def status():
    ...
    return {"service": "service_health_monitor", "domain": "monitoring",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "service_health_monitor", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting service_health_monitor port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
