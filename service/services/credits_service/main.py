"""
Credits Service — Payment Service

Template: svc_payment_002
Domain: payment | Tier: medium | Port: 8163
...
SERVICE_PORT        = config.get('port', 8163)
...
app = FastAPI(
    title="Credits Service API",
    description="Payment service | Port 8163 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "credits_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "credits_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "credits_service", "port": SERVICE_PORT,
...
async def payments():
    ...
    return {"service": "credits_service", "domain": "payment",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "credits_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting credits_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
