"""
Disaster Recovery Service — Backup Service

Template: svc_health_017
Domain: backup | Tier: small | Port: 8096
...
SERVICE_PORT        = config.get('port', 8096)
...
app = FastAPI(
    title="Disaster Recovery Service API",
    description="Backup service | Port 8096 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "disaster_recovery_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "disaster_recovery_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "disaster_recovery_service", "port": SERVICE_PORT,
...
async def backup_status():
    ...
    return {"service": "disaster_recovery_service", "domain": "backup",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "disaster_recovery_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting disaster_recovery_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
