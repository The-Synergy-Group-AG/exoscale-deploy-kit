"""Integration tests for disaster_recovery_service.
Port: 8096 | Tier: small | Namespace: jtp
...
class TestDisasterRecoveryServiceIntegration:
    ...
    def test_domain_endpoint_backup_status(self, base_url):
        """Integration: GET /backup/status responds correctly."""
        try:
            r = httpx.get(f"{base_url}/backup/status", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /backup/status returned {r.status_code}"
            data = r.json()
    ...
    def test_domain_endpoint_backup_create(self, base_url):
        """Integration: POST /backup/create responds correctly."""
        try:
            r = httpx.post(f"{base_url}/backup/create", json={}, timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /backup/create returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
    ...
    def test_domain_endpoint_backup_id(self, base_url):
        """Integration: GET /backup/{id} responds correctly."""
        try:
            r = httpx.get(f"{base_url}/backup/test-id", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /backup/{id} returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
