"""Unit tests for disaster_recovery_service.
Template: svc_health_017 | Port: 8096 | Tier: small
Namespace: jtp | Domain: backup
...
class TestDisasterRecoveryServiceUnit:
    """Unit tests — port 8096 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "disaster_recovery_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8096, f"Port mismatch: expected 8096 got {d['port']}"
    ...
    def test_root_returns_domain_and_template(self, client):
        ...
        assert r.status_code == 200
        d = r.json()
        assert d.get("domain") == "backup", f"Domain mismatch: {d}"
    ...
    def test_endpoint_get_backup_status(self, client):
        """Test GET /backup/status — status + schema validation"""
        response = client.get("/backup/status")
        assert response.status_code in (200, 201, 405, 422)
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "disaster_recovery_service", \
                f"Response must include service name, got: {data}"
            assert data.get("domain") == "backup", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/backup/status", \
    ...
    def test_endpoint_post_backup_create(self, client):
        """Test POST /backup/create — status + schema validation"""
        ...
        if response.status_code in (200, 201):
            ...
            assert data.get("domain") == "backup", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/backup/create", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
    ...
    def test_endpoint_get_backup_id(self, client):
        """Test GET /backup/{id} — status + schema validation"""
        response = client.get("/backup/{id}")
        assert response.status_code in (200, 201, 405, 422)
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "disaster_recovery_service", \
            ...
            assert data.get("domain") == "backup", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/backup/{id}", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
    ...
    def test_endpoint_post_restore(self, client):
        """Test POST /restore — status + schema validation"""
        ...
        if response.status_code in (200, 201):
            ...
            assert data.get("domain") == "backup", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/restore", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
    ...
    def test_endpoint_get_recovery_status(self, client):
        """Test GET /recovery/status — status + schema validation"""
        ...
        if response.status_code in (200, 201):
            ...
            assert data.get("domain") == "backup", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/recovery/status", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
    ...
    def test_endpoint_delete_backup_id(self, client):
        """Test DELETE /backup/{id} — status + schema validation"""
        response = client.delete("/backup/{id}")
        assert response.status_code in (200, 201, 405, 422)
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "disaster_recovery_service", \
