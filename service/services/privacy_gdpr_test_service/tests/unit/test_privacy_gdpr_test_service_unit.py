"""Unit tests for privacy_gdpr_test_service.
Template: svc_audit_014 | Port: 8255 | Tier: medium
...
class TestPrivacyGdprTestServiceUnit:
    """Unit tests — port 8255 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "privacy_gdpr_test_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8255, f"Port mismatch: expected 8255 got {d['port']}"
    ...
    def test_endpoint_get_compliance_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "privacy_gdpr_test_service", \
