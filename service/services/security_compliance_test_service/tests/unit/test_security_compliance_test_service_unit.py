"""Unit tests for security_compliance_test_service.
Template: svc_audit_014 | Port: 8211 | Tier: medium
...
class TestSecurityComplianceTestServiceUnit:
    """Unit tests — port 8211 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "security_compliance_test_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8211, f"Port mismatch: expected 8211 got {d['port']}"
    ...
    def test_endpoint_get_compliance_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "security_compliance_test_service", \
