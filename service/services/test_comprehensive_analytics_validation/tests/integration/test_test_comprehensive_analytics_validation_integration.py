"""Integration tests for test_comprehensive_analytics_validation.
Port: 8157 | Tier: small | Namespace: jtp
...
class TestTestComprehensiveAnalyticsValidationIntegration:
    ...
    def test_domain_endpoint_status(self, base_url):
        """Integration: GET /status responds correctly."""
        try:
            r = httpx.get(f"{base_url}/status", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /status returned {r.status_code}"
            data = r.json()
    ...
    def test_domain_endpoint_metrics_live(self, base_url):
        """Integration: GET /metrics/live responds correctly."""
        try:
            r = httpx.get(f"{base_url}/metrics/live", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /metrics/live returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
    ...
    def test_domain_endpoint_alerts_create(self, base_url):
        """Integration: POST /alerts/create responds correctly."""
        try:
            r = httpx.post(f"{base_url}/alerts/create", json={}, timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /alerts/create returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
