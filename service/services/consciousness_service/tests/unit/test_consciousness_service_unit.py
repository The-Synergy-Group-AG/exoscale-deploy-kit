"""Unit tests for consciousness_service.
Template: svc_health_017 | Port: 8235 | Tier: large
...
class TestConsciousnessServiceUnit:
    """Unit tests — port 8235 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "consciousness_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8235, f"Port mismatch: expected 8235 got {d['port']}"
    ...
    def test_endpoint_get_biological_harmony(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "consciousness_service", \
