"""Unit tests for test_search_functionality_validation.
Template: svc_monitoring_016 | Port: 8190 | Tier: small
...
class TestTestSearchFunctionalityValidationUnit:
    """Unit tests — port 8190 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "test_search_functionality_validation"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8190, f"Port mismatch: expected 8190 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "test_search_functionality_validation", \
