"""Unit tests for security_monitoring_system.
Template: svc_monitoring_016 | Port: 8203 | Tier: large
...
class TestSecurityMonitoringSystemUnit:
    """Unit tests — port 8203 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "security_monitoring_system"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8203, f"Port mismatch: expected 8203 got {d['port']}"
    ...
    def test_endpoint_get_security_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "security_monitoring_system", \
