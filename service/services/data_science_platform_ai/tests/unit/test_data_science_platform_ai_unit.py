"""Unit tests for data_science_platform_ai.
Template: svc_analytics_009 | Port: 8089 | Tier: medium
...
class TestDataSciencePlatformAiUnit:
    """Unit tests — port 8089 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "data_science_platform_ai"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8089, f"Port mismatch: expected 8089 got {d['port']}"
    ...
    def test_endpoint_get_analytics_dashboard(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "data_science_platform_ai", \
