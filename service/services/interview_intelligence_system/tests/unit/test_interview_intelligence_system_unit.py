"""Unit tests for interview_intelligence_system.
Template: svc_catalog_008 | Port: 8196 | Tier: medium
...
class TestInterviewIntelligenceSystemUnit:
    """Unit tests — port 8196 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "interview_intelligence_system"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8196, f"Port mismatch: expected 8196 got {d['port']}"
    ...
    def test_endpoint_get_jobs(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "interview_intelligence_system", \
