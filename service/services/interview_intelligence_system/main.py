"""
Interview Intelligence System — Career Service

Template: svc_catalog_008
Domain: career | Tier: medium | Port: 8196
...
SERVICE_PORT        = config.get('port', 8196)
...
app = FastAPI(
    ...
    description="Career service | Port 8196 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "interview_intelligence_system", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "interview_intelligence_system",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "interview_intelligence_system", "port": SERVICE_PORT,
...
async def jobs():
    ...
    return {"service": "interview_intelligence_system", "domain": "career",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "interview_intelligence_system", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting interview_intelligence_system port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
