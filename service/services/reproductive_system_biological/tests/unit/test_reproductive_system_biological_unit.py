"""Unit tests for reproductive_system_biological.
Template: svc_health_017 | Port: 8284 | Tier: large
...
class TestReproductiveSystemBiologicalUnit:
    """Unit tests — port 8284 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "reproductive_system_biological"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8284, f"Port mismatch: expected 8284 got {d['port']}"
    ...
    def test_endpoint_get_biological_harmony(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "reproductive_system_biological", \
