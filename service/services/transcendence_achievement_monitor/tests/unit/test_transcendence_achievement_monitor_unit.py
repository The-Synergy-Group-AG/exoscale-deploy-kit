"""Unit tests for transcendence_achievement_monitor.
Template: svc_reporting_005 | Port: 8168 | Tier: small
Namespace: jtp | Domain: gamification
...
class TestTranscendenceAchievementMonitorUnit:
    """Unit tests — port 8168 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "transcendence_achievement_monitor"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8168, f"Port mismatch: expected 8168 got {d['port']}"
    ...
    def test_root_returns_domain_and_template(self, client):
        ...
        assert r.status_code == 200
        d = r.json()
        assert d.get("domain") == "gamification", f"Domain mismatch: {d}"
    ...
    def test_endpoint_get_leaderboard(self, client):
        """Test GET /leaderboard — status + schema validation"""
        response = client.get("/leaderboard")
        assert response.status_code in (200, 201, 405, 422)
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "transcendence_achievement_monitor", \
                f"Response must include service name, got: {data}"
            assert data.get("domain") == "gamification", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/leaderboard", \
    ...
    def test_endpoint_post_achievements_unlock(self, client):
        """Test POST /achievements/unlock — status + schema validation"""
        response = client.post("/achievements/unlock")
        assert response.status_code in (200, 201, 405, 422)
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "transcendence_achievement_monitor", \
            ...
            assert data.get("domain") == "gamification", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/achievements/unlock", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
    ...
    def test_endpoint_get_xp(self, client):
        """Test GET /xp — status + schema validation"""
    ...
    def test_endpoint_get_badges(self, client):
        """Test GET /badges — status + schema validation"""
        ...
        if response.status_code in (200, 201):
            ...
            assert data.get("domain") == "gamification", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/badges", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
    ...
    def test_endpoint_get_points(self, client):
        """Test GET /points — status + schema validation"""
        ...
        if response.status_code in (200, 201):
            ...
            assert data.get("domain") == "gamification", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/points", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
    ...
    def test_endpoint_post_challenges_join(self, client):
        """Test POST /challenges/join — status + schema validation"""
        response = client.post("/challenges/join")
        assert response.status_code in (200, 201, 405, 422)
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "transcendence_achievement_monitor", \
            ...
            assert data.get("domain") == "gamification", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/challenges/join", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
