"""E2E tests for transcendence_achievement_monitor via gateway.
Port: 8168 | NS: jtp
...
class TestTranscendenceAchievementMonitorE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/transcendence_achievement_monitor/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/transcendence_achievement_monitor/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_leaderboard(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/transcendence_achievement_monitor/leaderboard", timeout=30.0)
    ...
    def test_gateway_endpoint_achievements_unlock(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/transcendence_achievement_monitor/achievements/unlock", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_xp(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/transcendence_achievement_monitor/xp", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_badges(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/transcendence_achievement_monitor/badges", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_points(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/transcendence_achievement_monitor/points", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_challenges_join(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/transcendence_achievement_monitor/challenges/join", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
