"""Unit tests for filtering_and_search_system.
Template: svc_workflow_012 | Port: 8139 | Tier: medium
...
class TestFilteringAndSearchSystemUnit:
    """Unit tests — port 8139 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "filtering_and_search_system"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8139, f"Port mismatch: expected 8139 got {d['port']}"
    ...
    def test_endpoint_get_workflows(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "filtering_and_search_system", \
