"""
Filtering And Search System — Workflow Service

Template: svc_workflow_012
Domain: workflow | Tier: medium | Port: 8139
...
SERVICE_PORT        = config.get('port', 8139)
...
app = FastAPI(
    title="Filtering And Search System API",
    description="Workflow service | Port 8139 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "filtering_and_search_system", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "filtering_and_search_system",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "filtering_and_search_system", "port": SERVICE_PORT,
...
async def workflows():
    ...
    return {"service": "filtering_and_search_system", "domain": "workflow",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "filtering_and_search_system", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting filtering_and_search_system port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
