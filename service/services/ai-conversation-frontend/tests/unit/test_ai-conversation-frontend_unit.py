"""Unit tests for ai-conversation-frontend.
Template: svc_frontend_003 | Port: 8281 | Tier: small
...
class TestAiConversationFrontendUnit:
    """Unit tests — port 8281 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "ai-conversation-frontend"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8281, f"Port mismatch: expected 8281 got {d['port']}"
    ...
    def test_endpoint_get_root(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "ai-conversation-frontend", \
