"""Unit tests for emotional_intelligence_advisor.
Template: svc_analytics_009 | Port: 8179 | Tier: large
...
class TestEmotionalIntelligenceAdvisorUnit:
    """Unit tests — port 8179 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "emotional_intelligence_advisor"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8179, f"Port mismatch: expected 8179 got {d['port']}"
    ...
    def test_endpoint_post_ai_process(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "emotional_intelligence_advisor", \
