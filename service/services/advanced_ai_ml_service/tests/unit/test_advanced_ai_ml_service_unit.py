"""Unit tests for advanced_ai_ml_service.
Template: svc_analytics_009 | Port: 8234 | Tier: large
...
class TestAdvancedAiMlServiceUnit:
    """Unit tests — port 8234 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "advanced_ai_ml_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8234, f"Port mismatch: expected 8234 got {d['port']}"
    ...
    def test_endpoint_post_ai_process(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "advanced_ai_ml_service", \
