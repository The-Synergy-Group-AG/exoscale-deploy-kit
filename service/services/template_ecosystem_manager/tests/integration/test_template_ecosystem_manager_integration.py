"""Integration tests for template_ecosystem_manager.
Port: 8197 | Tier: small | Namespace: jtp
...
class TestTemplateEcosystemManagerIntegration:
    ...
    def test_domain_endpoint_config(self, base_url):
        """Integration: GET /config responds correctly."""
        try:
            r = httpx.get(f"{base_url}/config", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /config returned {r.status_code}"
            data = r.json()
    ...
    def test_domain_endpoint_config(self, base_url):
        """Integration: PUT /config responds correctly."""
        try:
            r = httpx.post(f"{base_url}/config", json={}, timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /config returned {r.status_code}"
            data = r.json()
            assert data is not None
    ...
    def test_domain_endpoint_settings(self, base_url):
        """Integration: GET /settings responds correctly."""
        try:
            r = httpx.get(f"{base_url}/settings", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /settings returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
