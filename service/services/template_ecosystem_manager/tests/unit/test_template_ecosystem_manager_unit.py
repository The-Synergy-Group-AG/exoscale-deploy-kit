"""Unit tests for template_ecosystem_manager.
Template: svc_configuration_015 | Port: 8197 | Tier: small
Namespace: jtp | Domain: configuration
...
class TestTemplateEcosystemManagerUnit:
    """Unit tests — port 8197 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "template_ecosystem_manager"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8197, f"Port mismatch: expected 8197 got {d['port']}"
    ...
    def test_root_returns_domain_and_template(self, client):
        ...
        assert r.status_code == 200
        d = r.json()
        assert d.get("domain") == "configuration", f"Domain mismatch: {d}"
    ...
    def test_endpoint_get_config(self, client):
        """Test GET /config — status + schema validation"""
        response = client.get("/config")
        assert response.status_code in (200, 201, 405, 422)
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "template_ecosystem_manager", \
                f"Response must include service name, got: {data}"
            assert data.get("domain") == "configuration", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/config", \
    ...
    def test_endpoint_put_config(self, client):
        """Test PUT /config — status + schema validation"""
    ...
    def test_endpoint_get_settings(self, client):
        """Test GET /settings — status + schema validation"""
        ...
        if response.status_code in (200, 201):
            ...
            assert data.get("domain") == "configuration", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/settings", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
    ...
    def test_endpoint_post_settings_reset(self, client):
        """Test POST /settings/reset — status + schema validation"""
        ...
        if response.status_code in (200, 201):
            ...
            assert data.get("domain") == "configuration", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/settings/reset", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
    ...
    def test_endpoint_get_features(self, client):
        """Test GET /features — status + schema validation"""
        ...
        if response.status_code in (200, 201):
            ...
            assert data.get("domain") == "configuration", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/features", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
    ...
    def test_endpoint_put_features_name(self, client):
        """Test PUT /features/{name} — status + schema validation"""
        response = client.put("/features/{name}")
        assert response.status_code in (200, 201, 405, 422)
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "template_ecosystem_manager", \
            ...
            assert data.get("domain") == "configuration", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/features/{name}", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
