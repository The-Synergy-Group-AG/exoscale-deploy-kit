"""E2E tests for template_ecosystem_manager via gateway.
Port: 8197 | NS: jtp
...
class TestTemplateEcosystemManagerE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/template_ecosystem_manager/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/template_ecosystem_manager/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_config(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/template_ecosystem_manager/config", timeout=30.0)
    ...
    def test_gateway_endpoint_config(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/template_ecosystem_manager/config", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Gateway endpoint /config returned {r.status_code}"
            assert r.json() is not None
        except httpx.ConnectError:
    ...
    def test_gateway_endpoint_settings(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/template_ecosystem_manager/settings", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_settings_reset(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/template_ecosystem_manager/settings/reset", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_features(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/template_ecosystem_manager/features", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_features_name(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/template_ecosystem_manager/features/test-id", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
