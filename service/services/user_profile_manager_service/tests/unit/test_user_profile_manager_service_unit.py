"""Unit tests for user_profile_manager_service.
Template: svc_user_management_006 | Port: 8191 | Tier: medium
Namespace: jtp | Domain: user_management
...
class TestUserProfileManagerServiceUnit:
    """Unit tests — port 8191 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "user_profile_manager_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8191, f"Port mismatch: expected 8191 got {d['port']}"
    ...
    def test_root_returns_domain_and_template(self, client):
        ...
        assert r.status_code == 200
        d = r.json()
        assert d.get("domain") == "user_management", f"Domain mismatch: {d}"
    ...
    def test_endpoint_get_users(self, client):
        """Test GET /users — status + schema validation"""
        response = client.get("/users")
        assert response.status_code in (200, 201, 405, 422)
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "user_profile_manager_service", \
                f"Response must include service name, got: {data}"
            assert data.get("domain") == "user_management", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/users", \
    ...
    def test_endpoint_post_users(self, client):
        """Test POST /users — status + schema validation"""
    ...
    def test_endpoint_get_users_id(self, client):
        """Test GET /users/{id} — status + schema validation"""
        ...
        if response.status_code in (200, 201):
            ...
            assert data.get("domain") == "user_management", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/users/{id}", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
    ...
    def test_endpoint_put_users_id(self, client):
        """Test PUT /users/{id} — status + schema validation"""
    ...
    def test_endpoint_delete_users_id(self, client):
        """Test DELETE /users/{id} — status + schema validation"""
    ...
    def test_endpoint_get_users_id_profile(self, client):
        """Test GET /users/{id}/profile — status + schema validation"""
        response = client.get("/users/{id}/profile")
        assert response.status_code in (200, 201, 405, 422)
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "user_profile_manager_service", \
            ...
            assert data.get("domain") == "user_management", \
                f"Response must include domain, got: {data}"
            assert data.get("endpoint") == "/users/{id}/profile", \
                f"Response must include endpoint, got: {data}"
            assert data.get("template") is not None, "template key missing"
            assert data.get("template") != "unknown", \
                "Plan 123 violation: template must not be unknown"
