"""Integration tests for user_profile_manager_service.
Port: 8191 | Tier: medium | Namespace: jtp
...
class TestUserProfileManagerServiceIntegration:
    ...
    def test_domain_endpoint_users(self, base_url):
        """Integration: GET /users responds correctly."""
        try:
            r = httpx.get(f"{base_url}/users", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /users returned {r.status_code}"
            data = r.json()
    ...
    def test_domain_endpoint_users(self, base_url):
        """Integration: POST /users responds correctly."""
        try:
            r = httpx.post(f"{base_url}/users", json={}, timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /users returned {r.status_code}"
            data = r.json()
            assert data is not None
    ...
    def test_domain_endpoint_users_id(self, base_url):
        """Integration: GET /users/{id} responds correctly."""
        try:
            r = httpx.get(f"{base_url}/users/test-id", timeout=10.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Domain endpoint /users/{id} returned {r.status_code}"
            data = r.json()
            assert data is not None
            assert "service" in data, "Response must include service key"
        except httpx.ConnectError:
