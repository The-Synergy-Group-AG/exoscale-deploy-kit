"""User Story Acceptance Tests — user_profile_manager_service
US: US-001..US-0601 (241 stories) | Port: 8191 | Harmony: 0.997
...
SERVICE_BASE  = f"{GATEWAY_URL}/api/user_profile_manager_service"
SERVICE_PORT  = 8191


class TestUserProfileManagerServiceUserStories:
    ...
    def test_us_endpoints_operational(self):
        endpoints = ["/users", "/users", "/users/{id}", "/users/{id}", "/users/{id}", "/users/{id}/profile"]
        try:
    ...
    def test_endpoint_users(self):
        """GET /users must respond (200/201/400/422 valid)."""
        try:
            r = httpx.get(f"{SERVICE_BASE}/users", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Endpoint /users returned unexpected {r.status_code}"
            assert r.json() is not None
        except httpx.ConnectError:
            pytest.skip('Gateway not reachable')

    def test_endpoint_users(self):
        """POST /users must respond (200/201/400/422 valid)."""
        try:
            r = httpx.post(f"{SERVICE_BASE}/users", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Endpoint /users returned unexpected {r.status_code}"
            assert r.json() is not None
        except httpx.ConnectError:
    ...
    def test_endpoint_users_id(self):
        """GET /users/{id} must respond (200/201/400/422 valid)."""
        try:
            r = httpx.get(f"{SERVICE_BASE}/users/test-id", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Endpoint /users/{id} returned unexpected {r.status_code}"
            assert r.json() is not None
        except httpx.ConnectError:
            pytest.skip('Gateway not reachable')

    def test_endpoint_users_id(self):
        """PUT /users/{id} must respond (200/201/400/422 valid)."""
        try:
            r = httpx.post(f"{SERVICE_BASE}/users/test-id", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Endpoint /users/{id} returned unexpected {r.status_code}"
            assert r.json() is not None
        except httpx.ConnectError:
    ...
    def test_endpoint_users_id(self):
        """DELETE /users/{id} must respond (200/201/400/422 valid)."""
        try:
            r = httpx.post(f"{SERVICE_BASE}/users/test-id", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Endpoint /users/{id} returned unexpected {r.status_code}"
    ...
    def test_endpoint_users_id_profile(self):
        """GET /users/{id}/profile must respond (200/201/400/422 valid)."""
        try:
            r = httpx.get(f"{SERVICE_BASE}/users/test-id/profile", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
                f"Endpoint /users/{id}/profile returned unexpected {r.status_code}"
            assert r.json() is not None
        except httpx.ConnectError:
            pytest.skip('Gateway not reachable')
