"""Unit tests for service_registry.
Template: svc_monitoring_016 | Port: 8131 | Tier: small
...
class TestServiceRegistryUnit:
    """Unit tests — port 8131 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "service_registry"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8131, f"Port mismatch: expected 8131 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "service_registry", \
