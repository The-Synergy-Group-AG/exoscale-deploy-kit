"""
Service Registry — Monitoring Service

Template: svc_monitoring_016
Domain: monitoring | Tier: small | Port: 8131
...
SERVICE_PORT        = config.get('port', 8131)
...
app = FastAPI(
    ...
    description="Monitoring service | Port 8131 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "service_registry", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "service_registry",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "service_registry", "port": SERVICE_PORT,
...
async def status():
    ...
    return {"service": "service_registry", "domain": "monitoring",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "service_registry", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting service_registry port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
