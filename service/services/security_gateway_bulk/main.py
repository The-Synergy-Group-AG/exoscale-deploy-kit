"""
Security Gateway Bulk — Security Service

Template: svc_monitoring_016
Domain: security | Tier: large | Port: 8282
...
SERVICE_PORT        = config.get('port', 8282)
...
app = FastAPI(
    title="Security Gateway Bulk API",
    description="Security service | Port 8282 | Tier: large | NS: jtp",
...
async def root():
    return {
        "service": "security_gateway_bulk", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "security_gateway_bulk",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "security_gateway_bulk", "port": SERVICE_PORT,
...
async def security_status():
    ...
    return {"service": "security_gateway_bulk", "domain": "security",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "security_gateway_bulk", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting security_gateway_bulk port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
