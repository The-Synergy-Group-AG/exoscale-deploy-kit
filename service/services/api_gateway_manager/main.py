"""
Api Gateway Manager — Security Service

Template: svc_monitoring_016
Domain: security | Tier: large | Port: 8102
...
SERVICE_PORT        = config.get('port', 8102)
...
app = FastAPI(
    title="Api Gateway Manager API",
    description="Security service | Port 8102 | Tier: large | NS: jtp",
...
async def root():
    return {
        "service": "api_gateway_manager", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "api_gateway_manager",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "api_gateway_manager", "port": SERVICE_PORT,
...
async def security_status():
    ...
    return {"service": "api_gateway_manager", "domain": "security",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "api_gateway_manager", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting api_gateway_manager port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
