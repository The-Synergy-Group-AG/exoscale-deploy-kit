"""Unit tests for api_gateway_manager.
Template: svc_monitoring_016 | Port: 8102 | Tier: large
...
class TestApiGatewayManagerUnit:
    """Unit tests — port 8102 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "api_gateway_manager"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8102, f"Port mismatch: expected 8102 got {d['port']}"
    ...
    def test_endpoint_get_security_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "api_gateway_manager", \
