"""Unit tests for openai_api_test_service.
Template: svc_monitoring_016 | Port: 8146 | Tier: large
...
class TestOpenaiApiTestServiceUnit:
    """Unit tests — port 8146 | tier large | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "openai_api_test_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8146, f"Port mismatch: expected 8146 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "openai_api_test_service", \
