"""
Openai Api Test Service — Monitoring Service

Template: svc_monitoring_016
Domain: monitoring | Tier: large | Port: 8146
...
SERVICE_PORT        = config.get('port', 8146)
...
app = FastAPI(
    title="Openai Api Test Service API",
    description="Monitoring service | Port 8146 | Tier: large | NS: jtp",
...
async def root():
    return {
        "service": "openai_api_test_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "openai_api_test_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "openai_api_test_service", "port": SERVICE_PORT,
...
async def status():
    ...
    return {"service": "openai_api_test_service", "domain": "monitoring",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "openai_api_test_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting openai_api_test_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
