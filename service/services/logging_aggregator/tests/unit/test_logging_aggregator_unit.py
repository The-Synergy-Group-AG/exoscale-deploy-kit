"""Unit tests for logging_aggregator.
Template: svc_monitoring_016 | Port: 8229 | Tier: small
...
class TestLoggingAggregatorUnit:
    """Unit tests — port 8229 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "logging_aggregator"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8229, f"Port mismatch: expected 8229 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "logging_aggregator", \
