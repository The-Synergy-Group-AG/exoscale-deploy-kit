"""
Logging Aggregator — Monitoring Service

Template: svc_monitoring_016
Domain: monitoring | Tier: small | Port: 8229
...
SERVICE_PORT        = config.get('port', 8229)
...
app = FastAPI(
    title="Logging Aggregator API",
    description="Monitoring service | Port 8229 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "logging_aggregator", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "logging_aggregator",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "logging_aggregator", "port": SERVICE_PORT,
...
async def status():
    ...
    return {"service": "logging_aggregator", "domain": "monitoring",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "logging_aggregator", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting logging_aggregator port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
