"""E2E tests for performance_optimizer_category via gateway.
Port: 8129 | NS: jtp
...
class TestPerformanceOptimizerCategoryE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/performance_optimizer_category/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/performance_optimizer_category/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_status(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/performance_optimizer_category/status", timeout=30.0)
    ...
    def test_gateway_endpoint_metrics_live(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/performance_optimizer_category/metrics/live", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_alerts_create(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/performance_optimizer_category/alerts/create", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_logs(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/performance_optimizer_category/logs", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_performance(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/performance_optimizer_category/performance", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_alerts_id(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/performance_optimizer_category/alerts/test-id", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
