"""Unit tests for test_integration_database_postgresql.
Template: svc_monitoring_016 | Port: 8272 | Tier: small
...
class TestTestIntegrationDatabasePostgresqlUnit:
    """Unit tests — port 8272 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "test_integration_database_postgresql"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8272, f"Port mismatch: expected 8272 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "test_integration_database_postgresql", \
