"""Unit tests for integration_bridges_system.
Template: svc_workflow_012 | Port: 8186 | Tier: medium
...
class TestIntegrationBridgesSystemUnit:
    """Unit tests — port 8186 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "integration_bridges_system"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8186, f"Port mismatch: expected 8186 got {d['port']}"
    ...
    def test_endpoint_get_workflows(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "integration_bridges_system", \
