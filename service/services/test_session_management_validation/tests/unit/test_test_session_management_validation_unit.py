"""Unit tests for test_session_management_validation.
Template: svc_monitoring_016 | Port: 8143 | Tier: small
...
class TestTestSessionManagementValidationUnit:
    """Unit tests — port 8143 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "test_session_management_validation"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8143, f"Port mismatch: expected 8143 got {d['port']}"
    ...
    def test_endpoint_get_status(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "test_session_management_validation", \
