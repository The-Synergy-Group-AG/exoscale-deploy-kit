"""E2E tests for context_awareness_orchestrator via gateway.
Port: 8165 | NS: jtp
...
class TestContextAwarenessOrchestratorE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/context_awareness_orchestrator/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/context_awareness_orchestrator/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_workflows(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/context_awareness_orchestrator/workflows", timeout=30.0)
    ...
    def test_gateway_endpoint_workflows_execute(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/context_awareness_orchestrator/workflows/execute", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_workflows_id_status(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/context_awareness_orchestrator/workflows/test-id/status", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_workflows_id_cancel(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/context_awareness_orchestrator/workflows/test-id/cancel", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_pipelines(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/context_awareness_orchestrator/pipelines", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_pipelines_trigger(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/context_awareness_orchestrator/pipelines/trigger", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
