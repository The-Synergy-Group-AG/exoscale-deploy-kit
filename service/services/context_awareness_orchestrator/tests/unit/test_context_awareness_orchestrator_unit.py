"""Unit tests for context_awareness_orchestrator.
Template: svc_workflow_012 | Port: 8165 | Tier: medium
...
class TestContextAwarenessOrchestratorUnit:
    """Unit tests — port 8165 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "context_awareness_orchestrator"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8165, f"Port mismatch: expected 8165 got {d['port']}"
    ...
    def test_endpoint_get_workflows(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "context_awareness_orchestrator", \
