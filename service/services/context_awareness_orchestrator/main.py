"""
Context Awareness Orchestrator — Workflow Service

Template: svc_workflow_012
Domain: workflow | Tier: medium | Port: 8165
...
SERVICE_PORT        = config.get('port', 8165)
...
app = FastAPI(
    title="Context Awareness Orchestrator API",
    description="Workflow service | Port 8165 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "context_awareness_orchestrator", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "context_awareness_orchestrator",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "context_awareness_orchestrator", "port": SERVICE_PORT,
...
async def workflows():
    ...
    return {"service": "context_awareness_orchestrator", "domain": "workflow",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "context_awareness_orchestrator", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting context_awareness_orchestrator port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
