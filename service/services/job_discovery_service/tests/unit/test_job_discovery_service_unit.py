"""Unit tests for job_discovery_service.
Template: svc_catalog_008 | Port: 8267 | Tier: medium
...
class TestJobDiscoveryServiceUnit:
    """Unit tests — port 8267 | tier medium | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "job_discovery_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8267, f"Port mismatch: expected 8267 got {d['port']}"
    ...
    def test_endpoint_get_jobs(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "job_discovery_service", \
