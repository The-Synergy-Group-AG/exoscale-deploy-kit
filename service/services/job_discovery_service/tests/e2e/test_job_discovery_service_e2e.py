"""E2E tests for job_discovery_service via gateway.
Port: 8267 | NS: jtp
...
class TestJobDiscoveryServiceE2E:

    def test_gateway_health(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/job_discovery_service/health", timeout=30.0)
    ...
    def test_gateway_all_registries(self):
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/job_discovery_service/", timeout=30.0)
            assert r.status_code == 200
    ...
    def test_gateway_endpoint_jobs(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/job_discovery_service/jobs", timeout=30.0)
    ...
    def test_gateway_endpoint_applications(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/job_discovery_service/applications", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_interviews(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/job_discovery_service/interviews", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_cv_id(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/job_discovery_service/cv/test-id", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_career_advice(self):
        ...
        try:
            r = httpx.get(f"{GATEWAY_URL}/api/job_discovery_service/career/advice", timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
    ...
    def test_gateway_endpoint_cv_generate(self):
        ...
        try:
            r = httpx.post(f"{GATEWAY_URL}/api/job_discovery_service/cv/generate", json={}, timeout=30.0)
            assert r.status_code in (200, 201, 400, 422), \
