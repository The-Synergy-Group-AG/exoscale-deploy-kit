"""
Job Discovery Service — Career Service

Template: svc_catalog_008
Domain: career | Tier: medium | Port: 8267
...
SERVICE_PORT        = config.get('port', 8267)
...
app = FastAPI(
    title="Job Discovery Service API",
    description="Career service | Port 8267 | Tier: medium | NS: jtp",
...
async def root():
    return {
        "service": "job_discovery_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "job_discovery_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "job_discovery_service", "port": SERVICE_PORT,
...
async def jobs():
    ...
    return {"service": "job_discovery_service", "domain": "career",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "job_discovery_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting job_discovery_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
