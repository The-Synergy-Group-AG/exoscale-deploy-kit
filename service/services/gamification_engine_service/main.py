"""
Gamification Engine Service — Gamification Service

Template: svc_reporting_005
Domain: gamification | Tier: small | Port: 8263
...
SERVICE_PORT        = config.get('port', 8263)
...
app = FastAPI(
    title="Gamification Engine Service API",
    description="Gamification service | Port 8263 | Tier: small | NS: jtp",
...
async def root():
    return {
        "service": "gamification_engine_service", "type": "backend",
...
async def health():
    return {
        "status": "healthy", "service": "gamification_engine_service",
        "port": SERVICE_PORT, "resource_tier": RESOURCE_TIER,
        "kubernetes_namespace": KUBERNETES_NS,
        "timestamp": time.time(),
...
async def metrics():
    return {
        "service": "gamification_engine_service", "port": SERVICE_PORT,
...
async def leaderboard():
    ...
    return {"service": "gamification_engine_service", "domain": "gamification",
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(status_code=exc.status_code, content={
        "service": "gamification_engine_service", "error": exc.detail, "status_code": exc.status_code})


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting gamification_engine_service port=%d tier=%s ns=%s", SERVICE_PORT, RESOURCE_TIER, KUBERNETES_NS)
