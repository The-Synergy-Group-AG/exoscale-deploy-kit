"""Unit tests for gamification_engine_service.
Template: svc_reporting_005 | Port: 8263 | Tier: small
...
class TestGamificationEngineServiceUnit:
    """Unit tests — port 8263 | tier small | ns jtp"""
    ...
    def test_root_endpoint(self, client):
        ...
        d = r.json()
        assert d["service"] == "gamification_engine_service"
        assert d["template"] != "unknown", "Plan 123 violation: template=unknown"
        assert d["port"] == 8263, f"Port mismatch: expected 8263 got {d['port']}"
    ...
    def test_endpoint_get_leaderboard(self, client):
        ...
        if response.status_code in (200, 201):
            data = response.json()
            assert data.get("service") == "gamification_engine_service", \
